...
describe('normalizeName', () => {
  ...
  it('removes stopwords when requested', () => {
    expect(normalizeName('The User Profile', { removeStopwords: true })).toBe('user_profile');
...
describe('parseCanonicalKey', () => {
  it('parses valid canonical keys', () => {
    const parsed = parseCanonicalKey(`${NORMALIZATION_VERSION}|spec|rule|promotion_criteria`);
...
describe('Registry alias resolution', () => {
  it('resolves direct aliases', () => {
    ...
    const resolved = resolveCanonicalKey('norm-v1|spec|rule|session_state', registry);
...
describe('executeWeave', () => {
  ...
  describe('entity merging', () => {
    ...
    it('keeps entities with different roles separate (Phase 1)', () => {
      const entities: RawEntity[] = [
        ...
        createRawEntity({ cgId: 'cg_b', artifactRole: 'implementation', name: 'User' }),
      ];
  ...
  describe('statement remapping', () => {
    ...
    it('deduplicates statements with same subject/predicate/object', () => {
      const entities: RawEntity[] = [
        ...
        createRawEntity({ cgId: 'cg_b1', artifactId: 'docs/A', name: 'Active', type: 'state' }),
        createRawEntity({ cgId: 'cg_b2', artifactId: 'docs/B', name: 'Active', type: 'state' }),
      ];
      const statements: RawStatement[] = [
        createRawStatement({
          id: 'stmt_1',
  ...
  describe('evidence merging', () => {
    it('merges evidence IDs when entities merge', () => {
      ...
      expect(result.entities).toHaveLength(1);
      expect(result.entities[0].evidenceIds).toEqual(['ev_1', 'ev_2', 'ev_3', 'ev_4']);
    });
  });
  ...
  describe('stats tracking', () => {
    it('tracks merge statistics', () => {
      const entities: RawEntity[] = [
        createRawEntity({ cgId: 'cg_a', artifactId: 'docs/A', name: 'User' }),
        createRawEntity({ cgId: 'cg_b', artifactId: 'docs/B', name: 'User' }),
        createRawEntity({ cgId: 'cg_c', artifactId: 'docs/C', name: 'Profile' }),
      ];
      const statements: RawStatement[] = [
  ...
  describe('determinism', () => {
    it('produces same output for same input (rerun stability)', () => {
      const entities: RawEntity[] = [
        createRawEntity({ cgId: 'cg_fixed_a', artifactId: 'docs/A', name: 'User' }),
        createRawEntity({ cgId: 'cg_fixed_b', artifactId: 'docs/B', name: 'User' }),
      ];

      const result1 = executeWeave({ entities, statements: [], evidence: [] });
      const result2 = executeWeave({ entities, statements: [], evidence: [] });

      expect(result1.entities[0].canonicalId).toBe(result2.entities[0].canonicalId);
