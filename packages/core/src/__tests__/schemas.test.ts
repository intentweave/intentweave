...
describe('Schemas Module', () => {
  describe('SCHEMA_IDS', () => {
    it('should have all expected schema IDs', () => {
      ...
      expect(SCHEMA_IDS.LX_PROPOSALS).toBe('intentweave://schemas/lx-proposals/v1');
  ...
  describe('loadSchema', () => {
    ...
    it('should load lx-proposals schema', () => {
      const schema = loadSchema('LX_PROPOSALS') as { $id: string; title: string };
  ...
  describe('getSchemaIdUri', () => {
    it('should return correct URI for each type', () => {
      expect(getSchemaIdUri('RUN_META')).toBe('intentweave://schemas/run-meta/v1');
      expect(getSchemaIdUri('PX_GRAPH')).toBe('intentweave://schemas/px-graph/v1');
    });
  });
