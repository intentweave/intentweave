...
describe("NoopLedgerWriter", () => {
  ...
  describe("writeSnapshot", () => {
    ...
    it("should not throw on empty snapshot", async () => {
      ...
      await expect(
        writer.writeSnapshot(emptySnapshot, testContext),
  ...
  describe("touchAssertions", () => {
    it("should not throw", async () => {
      await expect(
        writer.touchAssertions(["lineage-1", "lineage-2"], "turn-001"),
      ).resolves.toBeUndefined();
    });

    it("should handle empty array", async () => {
      await expect(
        writer.touchAssertions([], "turn-001"),
      ).resolves.toBeUndefined();
    });
  });
  ...
  describe("neutralizeAssertions", () => {
    it("should not throw", async () => {
      await expect(
        writer.neutralizeAssertions(
          ["lineage-1"],
          "Superseded by new extraction",
          "turn-002",
        ),
      ).resolves.toBeUndefined();
    });
...
describe("LedgerWriter interface contract", () => {
  ...
  it("should support all LedgerStage values", () => {
    const stages: LedgerWriteContext["stage"][] = [
      "IN",
      "RX",
      "CX",
      "PX",
