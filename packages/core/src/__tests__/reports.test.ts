...
import { formatProblemsReport } from "../reports/formatters/problems-md.js";
import { formatFullReport } from "../reports/formatters/full-md.js";
import type {
  IssueFingerprint,
...
describe("Issue Fingerprinting", () => {
  ...
  describe("createNeedsReviewFingerprint", () => {
    it("creates fingerprint for ambiguity", () => {
      const fp = createNeedsReviewFingerprint(
  ...
  describe("createErrorFingerprint", () => {
    it("creates fingerprint for error", () => {
      const fp = createErrorFingerprint(
...
describe("Issue Registry", () => {
  ...
  describe("getOrAllocateIssueId", () => {
    it("allocates new ID for new fingerprint", () => {
      ...
      const result = getOrAllocateIssueId(
        registry,
        "test-session",
        "run-001",
        fingerprint,
    ...
    it("allocates correct prefix for each kind", () => {
      ...
      const cFp: ContradictionFingerprint = {
        kind: "contradiction",
        specClaimSourceKey: "s1",
        implObservationSourceKey: "i1",
      };
      const oFp: OpenEndFingerprint = {
        kind: "open_end",
        fromRole: "intent",
        toRole: "spec",
      };
      const nFp: NeedsReviewFingerprint = {
        kind: "needs_review",
        ambiguityType: "test",
      };
      const eFp: ErrorFingerprint = {
        kind: "error",
        errorCode: "E001",
        adapterName: "test",
        stage: "IN",
...
describe("Severity Computation", () => {
  describe("computeSeverity", () => {
    ...
    it("returns blocker for stageBreaking errors", () => {
      const severity = computeSeverity("error", 0.5, policy, {
        stageBreaking: true,
      });
    ...
    it("returns blocker for mustHave open_end with high confidence", () => {
      const severity = computeSeverity("open_end", 0.9, policy, {
        mustHave: true,
      });
      expect(severity).toBe("blocker");
    });
...
describe("Markdown Formatters", () => {
  const createMockReport = (issues: Issue[] = []): RunReport => ({
    ...
    inputs: {
      artifacts: [
        {
          id: "chat:specstory:test-session",
          type: "chat",
          source: "specstory",
        },
      ],
