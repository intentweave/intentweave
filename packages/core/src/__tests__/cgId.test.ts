...
describe("cgId utilities", () => {
  ...
  describe("Workspace ID functions", () => {
    ...
    describe("extractWorkspaceId", () => {
      it("extracts workspace ID from cgId", () => {
        expect(extractWorkspaceId("ws_1234|model|kg|entity/user")).toBe(
          "ws_1234",
        );
    ...
    describe("cgIdBelongsToWorkspace", () => {
      it("returns true when cgId belongs to workspace", () => {
        expect(
          cgIdBelongsToWorkspace("ws_1234|model|kg|entity/user", "ws_1234"),
        ).toBe(true);
      });

      it("returns false when cgId belongs to different workspace", () => {
        expect(
          cgIdBelongsToWorkspace("ws_1234|model|kg|entity/user", "ws_5678"),
        ).toBe(false);
      });
    });
    ...
    describe("hasStableWorkspaceRoot", () => {
      it("returns true for stable workspace roots", () => {
        expect(hasStableWorkspaceRoot("ws_1234|model|kg|entity/user")).toBe(
          true,
        );
      });

      it("returns false for legacy roots", () => {
        expect(hasStableWorkspaceRoot("cgchat|model|kg|entity/user")).toBe(
          false,
        );
      });
    });
    ...
    describe("migrateCgIdToWorkspace", () => {
      it("migrates cgId to new workspace", () => {
        const migrated = migrateCgIdToWorkspace(
          "cgchat|model|kg|entity/user",
          "ws_abcd",
        );
  ...
  describe("slugifySegment", () => {
    ...
    it("returns a deterministic fallback for empty/special-only input", () => {
      const emptyResult = slugifySegment("");
      expect(emptyResult).toMatch(/^_special_/);
      const specialResult = slugifySegment("!!!");
      expect(specialResult).toMatch(/^_special_/);
    });
  });
