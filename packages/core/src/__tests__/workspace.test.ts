  ...
  describe('Key → Path mapping', () => {
    describe('getWorkspacePath', () => {
      it('returns workspace directory path', () => {
        expect(getWorkspacePath('my-project')).toBe('.iw/workspaces/my-project');
      });

      it('uses custom base directory', () => {
        expect(getWorkspacePath('my-project', { baseDir: '/home/user/.iw' }))
    ...
    describe('getManifestPath', () => {
      it('returns manifest file path', () => {
        expect(getManifestPath('my-project')).toBe('.iw/workspaces/my-project/workspace.json');
      });
    });

    describe('getWorkspaceRunsPath', () => {
      it('returns runs directory path', () => {
        expect(getWorkspaceRunsPath('my-project')).toBe('.iw/workspaces/my-project/runs');
      });
    });

    describe('getRunPath', () => {
      it('returns specific run directory path', () => {
        expect(getRunPath('my-project', 'run-2026-01-10'))
          .toBe('.iw/workspaces/my-project/runs/run-2026-01-10');
      });
    });

    describe('getCuratedPath', () => {
      it('returns curated directory path', () => {
        expect(getCuratedPath('my-project')).toBe('.iw/workspaces/my-project/curated');
      });
    });
  });
  ...
  describe('WorkspaceManifest', () => {
    describe('createWorkspaceManifest', () => {
      it('creates valid manifest with defaults', () => {
        const manifest = createWorkspaceManifest('my-project', 'My Project', '/path/to/project');
      ...
      it('creates manifest with options', () => {
        const manifest = createWorkspaceManifest('my-project', 'My Project', '/path', {
    ...
    describe('upsertWorkspaceInRegistry', () => {
      ...
      it('updates existing workspace', () => {
        let registry: WorkspaceRegistry = {
          schemaVersion: '0.1',
          workspaces: [
            { key: 'existing', id: 'ws_old1', path: '/old' },
          ],
        };
