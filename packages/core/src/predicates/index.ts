 ...
 */
export const Predicates = Object.fromEntries(PREDICATES.map((p) => [p, p])) as {
  [K in Predicate]: K;
};
...
export type CanonicalPredicate =
  (typeof CANONICAL_PREDICATES)[keyof typeof CANONICAL_PREDICATES];
...
export type ExtractedPredicate =
  (typeof EXTRACTED_PREDICATES)[keyof typeof EXTRACTED_PREDICATES];
 ...
 */
export function isCanonicalPredicate(
  predicate: string,
): predicate is CanonicalPredicate {
  return Object.values(CANONICAL_PREDICATES).includes(
    predicate as CanonicalPredicate,
  );
}
 ...
 */
export function isExtractedPredicate(
  predicate: string,
): predicate is ExtractedPredicate {
  return Object.values(EXTRACTED_PREDICATES).includes(
    predicate as ExtractedPredicate,
  );
}
