...
export async function generateReport(
  ...
  const { importState, transcriptFingerprint } = await loadImportStateForReport(
    iwDir,
    runMeta,
  );
  ...
  const compactIssues =
    policy.maxIssues || policy.maxIssuesPerKind
  ...
  try {
    ...
    const aggregated: ReportImportState = {
      ...
      lastProcessedSeq: Math.max(
        ...sources.map((s) => s.lastProcessedSeq || 0),
      ),
...
function applyCompactMode(issues: Issue[], policy: ReportPolicy): Issue[] {
  ...
  if (policy.maxEvidencePerIssue || policy.maxExcerptChars) {
    result = result.map((issue) => {
      ...
      if (
        policy.maxEvidencePerIssue &&
        evidence.length > policy.maxEvidencePerIssue
      ) {
      ...
      if (policy.maxExcerptChars) {
        ...
        evidence = evidence.map((e) => ({
          ...e,
          excerpt:
            e.excerpt && e.excerpt.length > maxChars
...
async function convertFindingsToIssues(
  ...
  for (const finding of findings.findings) {
    ...
    const idResult = getOrAllocateIssueId(
      registry,
      sessionKey,
      runId,
      fingerprint,
...
function createFindingFingerprint(
  ...
  switch (kind) {
    ...
    case "open_end":
      return createOpenEndFingerprint(
        "spec",
        "implementation",
        finding.entities?.[0],
      );
    case "needs_review":
      return createNeedsReviewFingerprint(
        finding.category,
        finding.entities?.[0],
      );
    default:
...
export async function saveReport(
  ...
  await writeFile(
    join(archiveDir, `${report.run.id}.json`),
    JSON.stringify(report, null, 2),
  );
}
