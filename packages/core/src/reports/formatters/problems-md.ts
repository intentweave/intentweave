...
export function formatProblemsReport(report: RunReport): string {
  ...
  lines.push(
    "Prefer smallest fixes. If something is ambiguous, propose a role override command.",
  );
  lines.push(
    "Commands are provided inline with each issue — execute them directly.",
  );
  lines.push("-->");
  lines.push("");
  ...
  lines.push(
    `Mode: ${report.run.mode} (${formatDuration(report.run.durationMs)})`,
  );
  if (report.run.reuse) {
  ...
  if (blockers.length > 0) {
    ...
    // Other blockers
    const others = blockers.filter(
      (i) => i.kind !== "contradiction" && i.kind !== "error",
    );
  ...
  // Open ends
  const openEnds = report.issues.filter(
    (i) => i.kind === "open_end" && i.severity !== "blocker",
  );
...
function formatTldr(report: RunReport): string {
  ...
  if (report.summary.contradictions > 0) {
    parts.push(
      `🔴 ${report.summary.contradictions} contradiction${report.summary.contradictions > 1 ? "s" : ""} (spec ↔ implementation)`,
    );
  }
  if (report.summary.openEnds > 0) {
    parts.push(
      `🟠 ${report.summary.openEnds} open end${report.summary.openEnds > 1 ? "s" : ""} (missing link)`,
    );
  }
  if (report.summary.needsReview > 0) {
    parts.push(
      `🟡 ${report.summary.needsReview} needs-review item${report.summary.needsReview > 1 ? "s" : ""}`,
    );
  }
...
function formatDelta(report: RunReport): string {
  ...
  if (report.summary.trend) {
    ...
    if (newIssues > 0)
      parts.push(`+${newIssues} new issue${newIssues > 1 ? "s" : ""}`);
  ...
  if (report.delta) {
    if (report.delta.newMessages > 0) {
      parts.push(
        `+${report.delta.newMessages} new message${report.delta.newMessages > 1 ? "s" : ""} imported`,
      );
    }
    if (report.delta.roleOverrides > 0) {
      parts.push(
        `${report.delta.roleOverrides} role override${report.delta.roleOverrides > 1 ? "s" : ""}`,
      );
    }
...
function formatActionsTable(actions: SuggestedAction[]): string {
  ...
  for (const action of actions.slice(0, 10)) {
    ...
    lines.push(
      `| ${action.rank} | ${score} | ${action.description} | ${issue} | ${eta} |`,
    );
  }
