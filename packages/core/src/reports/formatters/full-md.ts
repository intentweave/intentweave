...
export function formatFullReport(report: RunReport): string {
  ...
  if (report.inputs.configHash) {
    lines.push(`| Config hash | \`${report.inputs.configHash.substring(0, 12)}...\` |`);
  }
  lines.push('');
  ...
  if (report.run.reuse) {
    ...
    const reusePercent = totalChunks > 0 ? ((reusedChunks / totalChunks) * 100).toFixed(0) : 0;
  ...
  if (Object.keys(report.summary.roleDistribution).length > 0) {
    ...
    lines.push('|------|-------|');
    for (const [role, count] of Object.entries(report.summary.roleDistribution)) {
      lines.push(`| ${role} | ${count} |`);
    }
    lines.push('');
  }
  ...
  lines.push('');
  lines.push(`- Intent → Spec: ${(report.summary.intentToSpecCoverage * 100).toFixed(0)}%`);
  lines.push(`- Spec → Implementation: ${(report.summary.specToImplCoverage * 100).toFixed(0)}%`);
  lines.push('');
  ...
  if (report.actions.length > 0) {
    ...
    for (const action of report.actions) {
      ...
      lines.push(`| ${action.rank} | ${score} | ${action.description} | ${action.issueId ?? '-'} | ${action.estimatedEffort ?? '-'} |`);
    }
    lines.push('');
  }
  ...
  if (Object.keys(report.generator.adapterVersions).length > 0) {
    ...
    for (const [adapter, version] of Object.entries(report.generator.adapterVersions)) {
  ...
  lines.push(`| blockerConfidence | ${report.inputs.reportPolicy.blockerConfidence} |`);
  lines.push(`| warningConfidence | ${report.inputs.reportPolicy.warningConfidence} |`);
  lines.push(`| includeKinds | ${report.inputs.reportPolicy.includeKinds.join(', ')} |`);
  lines.push(`| includeRoles | ${report.inputs.reportPolicy.includeRoles.join(', ')} |`);
  ...
  if (report.inputs.reportPolicy.maxEvidencePerIssue) {
    lines.push(`| maxEvidencePerIssue | ${report.inputs.reportPolicy.maxEvidencePerIssue} |`);
  }
  lines.push('');
...
function formatFullIssue(issue: Issue): string {
  ...
  // Header
  const severityIcon = { blocker: '🔴', warning: '🟠', info: '🟡' }[issue.severity];
