...
export async function generateOverview(
  ...
  if (existsSync(join(aggregateDir, "lx.proposals.json"))) {
    const lxData = JSON.parse(
      await readFile(join(aggregateDir, "lx.proposals.json"), "utf-8"),
    );
  ...
  if (existsSync(join(aggregateDir, "findings.json"))) {
    const findingsData = JSON.parse(
      await readFile(join(aggregateDir, "findings.json"), "utf-8"),
    );
  ...
  if (useJsonl) {
    ...
    for (const s of statements) {
      statementsStream.write(
        JSON.stringify({ _type: "statement", ...s }) + "\n",
      );
    }
