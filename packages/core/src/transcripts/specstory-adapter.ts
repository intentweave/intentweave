...
async function incrementalImport(
  ...
  // Parse slice
  const parseResult = parseSpecStorySlice(
    sliceBuffer,
    startOffset,
    state.sessionId,
  );
  ...
  const lastHeaderOffset =
    lastNewMsg.refs?.sourceLoc?.byteStart ?? state.lastProcessedHeaderOffset;

  const newState = updateImportState(
    state,
    buffer,
    newMessages.length,
    lastHeaderOffset,
...
export { SPECSTORY_ADAPTER_NAME, SPECSTORY_ADAPTER_VERSION };
