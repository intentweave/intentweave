...
import type { Profile, ShapeRule, ArtifactMapping } from '../pipeline/context.js';
...
export function mergeProfiles(base: Profile, override: Profile): Profile {
  return {
    ...
    artifactMappings: mergeArtifactMappings(base.artifactMappings, override.artifactMappings),
  ...
  for (const mapping of override) {
    ...
    if (existing) {
      byRole.set(mapping.role, {
        ...
        patterns: dedupeArray([...(existing.patterns ?? []), ...(mapping.patterns ?? [])]),
      });
    } else {
      byRole.set(mapping.role, mapping);
    }
