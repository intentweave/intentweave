...
export class TwoPassStrategy implements ExtractionStrategy {
  ...
  async extract(
    ...
    for (const chunk of chunks) {
      const result = await this.extractEntitiesFromChunk(chunk, schema, profile);
    ...
    console.log(`[TwoPass] Pass 1 complete: ${deduplicatedEntities.length} unique entities extracted`);
    ...
    if (deduplicatedEntities.length > 0) {
      ...
      console.log(`[TwoPass] Pass 2 complete: ${allStatements.length} statements extracted`);
    } else {
  ...
  private async extractEntitiesFromChunk(
    ...
    const userPrompt = buildExtractionPrompt(chunk, schema, profile, 'entities-only');
    ...
    if (response.finishReason === 'error') {
      console.error(`[TwoPass] Pass 1 error for chunk ${chunk.id}:`, response.error);
      return {
        entities: [],
        evidence: [],
        tokensUsed: 0,
    ...
    if (!parsed) {
      try {
        ...
        console.error(`[TwoPass] Failed to parse Pass 1 response for chunk ${chunk.id}`);
        return {
          entities: [],
          evidence: [],
  ...
  private async extractStatementsFromChunk(
    ...
    const userPrompt = buildExtractionPrompt(chunk, schema, profile, 'statements-only', entities);
    ...
    if (response.finishReason === 'error') {
      console.error(`[TwoPass] Pass 2 error for chunk ${chunk.id}:`, response.error);
      return {
        statements: [],
        tokensUsed: 0,
      };
    ...
    if (!parsed) {
      try {
        ...
        console.error(`[TwoPass] Failed to parse Pass 2 response for chunk ${chunk.id}`);
        return {
          statements: [],
          tokensUsed: response.tokensUsed.prompt + response.tokensUsed.completion,
        };
    ...
    const statements = (parsed.statements ?? [])
      ...
      .filter(s => !s.subjectCgId.startsWith('unknown:') && !(s.objectCgId ?? '').startsWith('unknown:'));
