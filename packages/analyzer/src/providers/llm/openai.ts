...
const MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
  ...
  'gpt-5.1-codex-mini': { maxTokens: 256000, supportsJsonSchema: true, useCompletionTokensParam: true, fixedTemperature: 1 },
  'gpt-5.1': { maxTokens: 256000, supportsJsonSchema: true, useCompletionTokensParam: true, fixedTemperature: 1 },
  'gpt-5-mini': { maxTokens: 256000, supportsJsonSchema: true, useCompletionTokensParam: true, fixedTemperature: 1 },
  'gpt-5': { maxTokens: 256000, supportsJsonSchema: true, useCompletionTokensParam: true, fixedTemperature: 1 },
...
export class OpenAILLMProvider implements LLMProvider {
  ...
  private readonly config: Required<Pick<OpenAIConfig, 'model' | 'defaultTemperature' | 'defaultMaxTokens' | 'timeoutMs'>> & OpenAIConfig;
  ...
  get capabilities(): LLMProviderCapabilities {
    ...
    const modelCaps = MODEL_CAPABILITIES[model] ?? MODEL_CAPABILITIES[DEFAULT_MODEL];
  ...
  async complete(request: LLMRequest): Promise<LLMResponse> {
    ...
    try {
      ...
      const temperatureValue = getModelTemperature(model, request.temperature ?? this.config.defaultTemperature);
 ...
 */
export function createOpenAILLMProvider(config?: OpenAIConfig): OpenAILLMProvider {
