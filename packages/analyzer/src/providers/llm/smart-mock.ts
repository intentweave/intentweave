...
const debugLog = process.env.DEBUG_SMART_MOCK ? console.log.bind(console) : () => {};
  ...
  private extractFromText(text: string): {
    ...
    while ((match = rolePattern.exec(text)) !== null) {
      try {
        ...
        if (!scope || !roleName || typeof scope !== 'string' || typeof roleName !== 'string') {
          continue;
        }
        ...
        if (!entities.some(e => e.name === fullName)) {
          ...
          debugLog(`[SmartMock Debug] buildCgId args:`, {
            ...
            segment, segmentType: typeof segment,
            rootValue, rootValueType: typeof rootValue
          });
          
          entities.push({
            cgId: buildCgId(entityType, segment, { root: rootValue, kind: 'role' }),
            name: fullName,
            kind: 'role',
        ...
        console.error(`[SmartMock] Error building cgId for role "${match[1]}:${match[2]}":`, roleError);
        throw roleError;
      }
    }
    
    // Extract actions (read, write, delete, admin, create, manage, etc.)
    const actionWords = ['read', 'write', 'delete', 'admin', 'create', 'manage', 'view', 'edit', 'approve', 'assign', 'remove', 'update'];
      ...
      if (regex.test(text)) {
        if (!entities.some(e => e.name === action)) {
          debugLog(`[SmartMock Debug Action] buildCgId args:`, {
            entityType: 'action', segment: action, segmentType: typeof action,
            rootValue: this.config.workspaceKey, rootType: typeof this.config.workspaceKey
          });
          entities.push({
            cgId: buildCgId('action', action, { root: this.config.workspaceKey, kind: 'action' }),
            name: action,
            kind: 'action',
    ...
    const resourceWords = ['document', 'project', 'workspace', 'user', 'member', 'resource', 'permission', 'role', 'assignment'];
      ...
      if (regex.test(text)) {
        if (!entities.some(e => e.name === resource)) {
          debugLog(`[SmartMock Debug Resource] buildCgId args:`, {
            entityType: 'resource', segment: resource, segmentType: typeof resource,
            rootValue: this.config.workspaceKey, rootType: typeof this.config.workspaceKey
          });
          entities.push({
            cgId: buildCgId('resource', resource, { root: this.config.workspaceKey, kind: 'resource' }),
            name: resource,
            kind: 'resource',
    ...
    const stateWords = ['pending', 'approved', 'active', 'inactive', 'archived', 'deleted', 
                        'requested', 'suspended', 'retired', 'completed', 'draft', 'published'];
      ...
      if (regex.test(text)) {
        if (!entities.some(e => e.name === state)) {
          debugLog(`[SmartMock Debug State] buildCgId args:`, {
            entityType: 'state', segment: state, segmentType: typeof state,
            rootValue: this.config.workspaceKey, rootType: typeof this.config.workspaceKey
          });
          entities.push({
            cgId: buildCgId('state', state, { root: this.config.workspaceKey, kind: 'state' }),
            name: state,
            kind: 'state',
    ...
    while ((match = eventPattern.exec(text)) !== null) {
      ...
      if (!entities.some(e => e.name === eventName)) {
        entities.push({
          cgId: buildCgId('event', eventName, { root: this.config.workspaceKey, kind: 'event' }),
          name: eventName,
          kind: 'event',
    ...
    while ((match = dottedActionPattern.exec(text)) !== null) {
      ...
      if (!entities.some(e => e.name === actionName)) {
        entities.push({
          cgId: buildCgId('action', actionName, { root: this.config.workspaceKey, kind: 'action' }),
          name: actionName,
          kind: 'action',
    ...
    while ((match = requirementPattern.exec(text)) !== null) {
      ...
      debugLog(`[SmartMock Debug Requirement] buildCgId args:`, {
        entityType: 'requirement', segment: shortName, segmentType: typeof shortName,
        rootValue: this.config.workspaceKey, rootType: typeof this.config.workspaceKey
      });
      
      entities.push({
        cgId: buildCgId('requirement', shortName, { root: this.config.workspaceKey, kind: 'requirement' }),
        name: shortName,
        kind: 'requirement',
    ...
    debugLog(`[SmartMock Debug] Starting relationship generation... Entities: ${entities.length}`);
    
    // Generate relationships
    
    // Pattern: State transitions "from → to (via action)" or "from -> to (via action)"
    const transitionPattern = /\b([a-z]+)\s*(?:→|->)\s*([a-z]+)\s*\(via\s+([a-z._]+)\)/gi;
    while ((match = transitionPattern.exec(text)) !== null) {
      const fromState = match[1];
      const toState = match[2];
      const action = match[3];
      
      const fromEntity = entities.find(e => e.name === fromState && e.kind === 'state');
      const toEntity = entities.find(e => e.name === toState && e.kind === 'state');
    ...
    while ((match = roleCan.exec(text)) !== null) {
      ...
      const actionEntity = entities.find(e => e.name === actionName || e.name.endsWith(`.${actionName}`));
      ...
      if (actionEntity) {
        ...
        if (!roleEntity) {
          roleEntity = {
            cgId: buildCgId('role', 'authorized-role', { root: this.config.workspaceKey, kind: 'role' }),
    ...
    const canPattern = /\b([a-z:_-]+)\s+(?:can|grants?|allows?)\s+([a-z]+)(?:\s+([a-z]+))?\b/gi;
    while ((match = canPattern.exec(text)) !== null) {
      debugLog(`[SmartMock Debug] CAN pattern match:`, { role: match[1], action: match[2], resource: match[3] });
      ...
      debugLog(`[SmartMock Debug] Entity lookup:`, { 
        roleName, roleFound: !!roleEntity, roleHasCgId: !!roleEntity?.cgId, roleCgId: roleEntity?.cgId,
        actionName, actionFound: !!actionEntity, actionHasCgId: !!actionEntity?.cgId, actionCgId: actionEntity?.cgId
      });
    ...
    const inheritsPattern = /\b([a-z:_-]+)\s+inherits?\s+(?:from|permissions from)\s+([a-z:_-]+)\b/gi;
    while ((match = inheritsPattern.exec(text)) !== null) {
      debugLog(`[SmartMock Debug] INHERITS match:`, { child: match[1], parent: match[2] });
    ...
    while ((match = hasPattern.exec(text)) !== null) {
      debugLog(`[SmartMock Debug] HAS match:`, { source: match[1], target: match[2] });
      ...
      debugLog(`[SmartMock Debug] HAS entity lookup:`, {
        sourceName, sourceFound: !!sourceEntity, sourceCgId: sourceEntity?.cgId, sourceCgIdType: typeof sourceEntity?.cgId,
        targetName, targetFound: !!targetEntity, targetCgId: targetEntity?.cgId, targetCgIdType: typeof targetEntity?.cgId
      });
    ...
    const managesPattern = /\b([a-z:_-]+)\s+(?:manages?|controls?)\s+([a-z]+)\b/gi;
    while ((match = managesPattern.exec(text)) !== null) {
      debugLog(`[SmartMock Debug] MANAGES match:`, { source: match[1], target: match[2] });
      const sourceName = match[1];
      const targetName = match[2];
