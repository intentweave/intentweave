...
export function findingToIssue(
  ...
  return {
    ...
    evidence: finding.entities.map((e) => ({
...
function generateJumpTargets(
  ...
  const relatedStatements = provenance.statements.filter(
    (s) => entitySet.has(s.subject) || entitySet.has(s.object),
  );

  let stmtIdx = 1;
  for (const s of relatedStatements.slice(0, 20)) {
    // Limit to 20 statements
    const subLabel = s.subject.split("/").pop() || s.subject;
    const objLabel = s.object.split("/").pop() || s.object;
    const artifactPath =
      entityMap.get(s.subject)?.artifactPath ||
      entityMap.get(s.object)?.artifactPath;
 ...
 */
function generateHints(issue: VizIssue, provenance: ProvenanceInfo): string[] {
  const hints: string[] = [];
  ...
  if (ruleId === "consistency-040") {
    ...
    if (actionEntities.length > 0 && roleNames.length > 0) {
      ...
      for (const actionCgId of actionEntities.slice(0, 3)) {
        ...
        const formattedAction = actionLabel
          .replace(/-/g, " ")
      ...
      hints.push(
        "Define roles in your requirements document, then map actions to them.",
      );
      hints.push("");
      hints.push("Example:");
      hints.push("```");
      ...
      hints.push(
        "User can " + (actionEntities[0]?.split("/").pop() || "DoSomething"),
      );
      hints.push("```");
    } else {
      // Unknown action format - generic hint
      hints.push(
        "Ensure this action is authorized for at least one role in your requirements.",
      );
    }
  }
  ...
  // validation-005: States not connected to transitions
    ...
    if (stateEntities.length > 0) {
      ...
      hints.push("");
      hints.push(
        "Connect these states to transitions by defining state changes:",
      );
      hints.push("");
      ...
      for (const stateCgId of stateEntities.slice(0, 3)) {
        ...
        hints.push(
          `- Define an action that transitions **to** or **from** \`${stateLabel}\``,
        );
      }
      ...
      hints.push(
        `- Action "DoSomething" moves Task from "Open" to "${exampleState}"`,
      );
      hints.push("```");
    }
  }
...
function generateDescription(
  ...
  if (entityCount > 0) {
    ...
    if (entityCount <= 10) {
      ...
      lines.push("<details>");
      lines.push(
        `<summary><strong>Involved Entities (${entityCount})</strong></summary>`,
      );
      lines.push("");
      for (const cgId of issue.entities) {
        const label = cgId.split("/").pop() || cgId;
    ...
    if (provenance && provenance.statements.length > 0) {
      ...
      if (relatedStatements.length > 0) {
        ...
        for (const [artifact, stmts] of byArtifact) {
          const artifactPath =
            provenance.entities.find((e) => e.artifact === artifact)
      ...
      if (jumpTargets.length > 0) {
        ...
        if (entityTargets.length > 0) {
          ...
          for (const t of entityTargets) {
            if (t.artifactPath) {
              lines.push(
                `- [${t.id}] \`${t.label}\` — [${t.artifactPath}](${t.artifactPath})`,
              );
            } else {
        ...
        if (stmtTargets.length > 0) {
          ...
          for (const t of stmtTargets.slice(0, 10)) {
...
export function generateIssueLens(
  ...
  const subgraph = buildSubgraph(issue.entities, entities, statements, {
    depth,
    maxNodes,
    includeGhostEdges,
...
export function generateEntityLens(
  ...
  const subgraph = buildSubgraph([entityId], entities, statements, {
    depth,
    maxNodes,
...
function generateEntityDescription(
  entity: VizEntity,
  subgraph: import("./subgraph.js").Subgraph,
): string {
  ...
  if (connectedNodes.length > 0) {
    ...
    for (const [type, nodes] of byType) {
      ...
      lines.push(
        `- ${icon} **${type}s** (${nodes.length}): ${nodes
          .slice(0, 5)
          .map((n) => `\`${n.label || n.id.split("/").pop()}\``)
          .join(", ")}${nodes.length > 5 ? "..." : ""}`,
      );
    }
  }
