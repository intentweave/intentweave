...
export function getPredicateLabel(predicate: string): string {
  return (
    PREDICATE_LABELS[predicate] || predicate.toLowerCase().replace(/_/g, " ")
  );
}
...
export function buildSubgraph(
  ...
  if (includeGhostEdges && issueKind && issueTitle) {
    const ghostEdges = generateGhostEdges(
      nodes,
      issueKind,
      issueTitle,
      nodeIdMap,
...
function generateGhostEdges(
  ...
  if (
    titleLower.includes("missing a source state") ||
    titleLower.includes("missing source")
  ) {
  ...
  if (
    titleLower.includes("missing a target state") ||
    titleLower.includes("missing target")
  ) {
    const transitionNodes = nodes.filter((n) => n.type === "transition");
    for (const transition of transitionNodes) {
