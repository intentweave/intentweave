...
export const ISO_DURATION_PATTERN =
  /P(\d+Y)?(\d+M)?(\d+W)?(\d+D)?(T(\d+H)?(\d+M)?(\d+S)?)?/gi;

/** Number + unit pattern */
export const NUMBER_UNIT_PATTERN =
  /(\d+)\s*(year|years|month|months|week|weeks|day|days|hour|hours|yr|yrs|mo|mos|wk|wks|hr|hrs)/gi;
...
export function extractTransitions(
    ...
    if (parsed) {
      ...
      if (confidence >= (opts.minConfidence || 0.5)) {
        ...
        const existingIdx = hits.findIndex(
          (h) => h.from === hit.from && h.to === hit.to,
        );
        if (existingIdx >= 0) {
          hits[existingIdx].evidence = mergeEvidenceLists(
            hits[existingIdx].evidence,
            hit.evidence,
          );
          hits[existingIdx].confidence = Math.max(
            hits[existingIdx].confidence,
            hit.confidence,
          );
        } else {
          hits.push(hit);
  ...
  for (const statement of snapshot.statements) {
    if (statement.predicate === "TRANSITIONS_TO") {
      const fromEntity = snapshot.entities.find(
        (e) => e.cgId === statement.subjectCgId,
      );
      ...
      if (
        fromEntity &&
        toEntity &&
        statement.confidence >= (opts.minConfidence || 0)
      ) {
        const existingIdx = hits.findIndex(
          (h) => h.from === fromEntity.name && h.to === toEntity.name,
        );
        if (existingIdx >= 0) {
          hits[existingIdx].evidence = mergeEvidenceLists(
            hits[existingIdx].evidence,
 ...
 */
export function findStateMentions(
  text: string,
  dictionary: Map<string, string>,
): string[] {
...
export function detectAbsence(
  text: string,
): { action: string; negated: boolean } | null {
...
export function detectDuration(
  text: string,
): { iso: string; unit?: DurationUnit; value?: number } | null {
...
function inferDefaultFromState(
  to: string,
  dictionary: Map<string, string>,
): string | null {
