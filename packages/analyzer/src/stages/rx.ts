...
const DEFAULT_SCHEMA: EntitySchema = {
  ...
  predicates: ['ROLE_CAN', 'HAS_STATE', 'TRANSITIONS_TO', 'TRIGGERED_BY', 'REQUIRES', 'CONTAINS', 'IMPLEMENTS'],
};
...
export async function runRxStage(
  ...
  const context: ContextBundle = hooks.context 
    ? await hooks.context.getContext(input.artifactId, input.meta as Record<string, unknown>)
    : {};
  ...
  while (attempt <= budget.maxRetries) {
    try {
      ...
      // ────────────────────────────────────────────────────────────────────────
      const refResult = resolveStatementRefs(result.entities, result.statements);
      ...
      if (attempt < budget.maxRetries && budget.shouldRetry(lastError, attempt)) {
