...
export function detectArtifactRole(filePath: string, content: string): string {
  ...
  if (filePath.endsWith(".md")) {
    ...
    if (
      /^#.*\b(intent|goal|objective|prompt|requirements?)\b/im.test(firstLines)
    ) {
      return "intent";
    }
...
function flushCurrentChunk(builder: ChunkBuilder, artifactId: string): void {
  if (builder.currentChunk && builder.currentChunk.content) {
    const chunk: SemanticChunk = {
      id:
        builder.currentChunk.id ?? `${artifactId}-chunk-${builder.chunkIndex}`,
 ...
 */
function tableToStructuredSummary(
  table: ParsedTable,
  caption?: string,
): string {
  const lines: string[] = [];
      ...
      for (const subBlock of subChunks) {
        ...
        chunks.push({
          ...
          startLine:
            lineNumber +
            (
              content.slice(charOffset, charOffset + subOffset).match(/\n/g) ||
              []
            ).length,
          endLine:
            lineNumber +
            (
              content
                .slice(charOffset, charOffset + subOffset + subBlock.length)
                .match(/\n/g) || []
            ).length,
...
export async function runInStage(
  ...
  const detectedRole =
    input.artifactRole ?? detectArtifactRole(filePath, content);
