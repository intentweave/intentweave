...
function inferResourceFromStates(
  ...
  for (const state of states) {
    ...
    const stateIdx = cgIdParts.findIndex(
      (p) => p === "state" || p.startsWith("state"),
    );
    ...
    if (stateIdx >= 0 && cgIdParts.length > stateIdx + 2) {
      ...
      if (!isLikelyStateName(resourceName)) {
        resourceCandidates.set(
          resourceName,
          (resourceCandidates.get(resourceName) ?? 0) + 1,
        );
      }
    }
  }
  ...
  if (!bestCandidate) {
    ...
    const potentialResource = artifactParts.find(
      (p) =>
        p.length > 2 &&
        !isLikelyStateName(p) &&
        !["ws", "model", "kg", "prompt", "run"].includes(p.toLowerCase()),
    );
  ...
  if (!bestCandidate) {
    ...
    console.log(
      '[MX] Using generic "entity" as inferred resource name (no context available)',
    );
  }
  ...
  console.log(
    `[MX] Inferred resource '${bestCandidate}' from ${states.length} state entities`,
  );
...
function synthesizeHasStateStatements(
  ...
  if (!resourceReferencedInStatements && entities.length > states.length + 1) {
    console.warn(
      `[MX] Skipping HAS_STATE synthesis: resource '${resource.name}' not referenced in statements`,
    );
    return { statements: [] };
  }
  ...
  console.log(
    `[MX] Synthesizing ${unboundStates.length} HAS_STATE statements (single resource fallback: ${resource.name})`,
  );

  // Synthesize HAS_STATE statements for unbound states
  const synthesized = unboundStates.map(
    (state): Statement => ({
...
function findAssociatedResource(
  ...
  if (hasStateStmt) {
    ...
    const resource = entities.find(
      (e) => e.cgId.toLowerCase() === normalizedSubjectCgId,
    );
  ...
  if (cgIdParts.length >= 2) {
    ...
    const resource = entities.find(
      (e) =>
        e.type === "resource" && e.name.toLowerCase() === potentialResourceName,
    );
    if (resource) return resource;
  }
...
function remapStatementCgIds(
  ...
  return statements.map((stmt) => {
    ...
    const newObject = stmt.objectCgId
...
function findEntitiesByType(type: string, entities: Entity[]): Entity[] {
  return entities.filter(
    (e) => e.type === type || e.props?.inferredKind === type,
  );
}
 ...
 */
function generateTransitionCgIdFallback(
  artifactId: string,
  index: number,
): string {
...
function materializeFromStatements(
  ...
  entities: Entity[];
  statements: Statement[];
  loweredStatementIds: Set<string>;
} {
  ...
  for (const stmt of transitionStmts) {
    ...
    const toEntity = stmt.objectCgId
      ? findEntity(stmt.objectCgId, entities)
      : undefined;
    ...
    if (!resource && toEntity.type === "state") {
      ...
      const hasStateForTarget = statements.find(
        (s) =>
          s.predicate === TRANSITION_PREDICATES.HAS_STATE &&
          s.objectCgId === toEntity.cgId,
      );
      if (hasStateForTarget) {
        resource =
          findEntity(hasStateForTarget.subjectCgId, entities) ?? undefined;
      }
    }
    ...
    const triggerStmts = statements.filter(
      (s) =>
        s.predicate === TRANSITION_PREDICATES.TRIGGERS &&
        (s.objectCgId === stmt.subjectCgId || s.objectCgId === stmt.objectCgId),
    );
    const triggerName =
      triggerStmts.length > 0
    ...
    const transitionCgId = buildTransitionCgId(
      artifactId,
      resourceName,
      isActionTriggered && sourceStates.length > 0
        ? sourceStates[0].name
        : fromEntity.name,
      toEntity.name,
...
function inferTransitionsFromEntities(
  ...
  for (const event of events) {
    const eventText = [event.name, JSON.stringify(event.props ?? {})]
      .join(" ")
      .toLowerCase();
...
function validateCanonicalSemantics(
  ...
  for (const stmt of statements) {
    ...
    const object = stmt.objectCgId
      ? entityByCgId.get(stmt.objectCgId)
      : undefined;
...
export async function runMxStage(
  ...
  const synthesisResult = synthesizeHasStateStatements(
    entitiesWithResource,
    cxStatements,
    artifactId,
  );
  const statementsWithHasState = [
    ...cxStatements,
    ...synthesisResult.statements,
  ];

  // Step 1: Prefix state entities with their associated resource
  const prefixed = prefixStatesWithResource(
    entitiesWithResource,
    statementsWithHasState,
  );
  const prefixedStatements = remapStatementCgIds(
    statementsWithHasState,
    prefixed.cgIdRemap,
  );
  ...
  if (prefixed.cgIdRemap.size > 0) {
    console.log(
      `[MX] Prefixed ${prefixed.cgIdRemap.size} state entities with resource names`,
    );
  }
  ...
  if (explicit.loweredStatementIds.size > 0) {
    allStatements = allStatements.filter(
      (s) =>
        s.predicate !== TRANSITION_PREDICATES.TRANSITIONS_TO ||
        !s.id ||
        !explicit.loweredStatementIds.has(s.id),
    );
    console.log(
      `[MX] Lowered ${explicit.loweredStatementIds.size} TRANSITIONS_TO statements to canonical form`,
    );
  }
  ...
  if (triggeredByStmts.length > 0) {
    ...
    allStatements = allStatements.filter(
      (s) =>
        s.predicate !== TRANSITION_PREDICATES.TRIGGERED_BY ||
        !s.id ||
    ...
    console.log(
      `[MX] Inverted ${triggeredByStmts.length} TRIGGERED_BY statements to canonical TRIGGERS`,
    );
  }
  ...
  const canonicalValidation = validateCanonicalSemantics(
    allEntities,
    allStatements,
  );
  ...
  const unresolvedRefs =
    preGate.unresolvedStats.total > 0
      ? {
          total: preGate.unresolvedStats.total,
          byPredicate: preGate.unresolvedStats.byPredicate,
        }
