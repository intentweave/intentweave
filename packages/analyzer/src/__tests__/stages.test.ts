 ...
 */
function createEntity(partial: Partial<Entity> & { cgId: string; name: string }): Entity {
  return {
...
function createStatement(
  partial: Partial<Statement> & { id: string; subjectCgId: string; predicate: string }
): Statement {
  return {
    id: partial.id,
 ...
 */
function createTestContext(profileOverrides: Partial<Profile> = {}): PipelineContext {
  ...
  return {
    ...
    generateCgId: (type, name) => `cg:test:${type}:${name.toLowerCase().replace(/\s+/g, '-')}`,
  };
}
...
describe('CX Stage', () => {
  ...
  describe('runCxStage', () => {
    ...
    it('preserves evidence from RX', async () => {
      ...
      const entities = [createEntity({ cgId: 'cg:1', name: 'Admin', evidence: ['ev:1'] })];
  ...
  describe('utility functions', () => {
    ...
    it('getStatements returns statements', async () => {
      const statements = [
        createStatement({ id: 'stmt:1', subjectCgId: 'cg:1', predicate: 'HAS_STATE' }),
      ];
...
describe('MX Stage', () => {
  ...
  describe('runMxStage', () => {
    it('returns valid MX output with schema', async () => {
      const entities = [createEntity({ cgId: 'cg:1', name: 'State A', type: 'state' })];
    ...
    it('tracks orphan entities (not in transitions)', async () => {
      const entities = [
        createEntity({ cgId: 'cg:state-a', name: 'State A', type: 'state' }),
        createEntity({ cgId: 'cg:state-b', name: 'State B', type: 'state' }),
        createEntity({ cgId: 'cg:orphan', name: 'Orphan Entity', type: 'concept' }),
      ];
      const statements = [
        createStatement({
          id: 'stmt:trans',
    ...
    it('preserves original entities and statements', async () => {
      const entities = [
        createEntity({ cgId: 'cg:1', name: 'Entity 1' }),
      ];
      const statements = [
        createStatement({ id: 'stmt:1', subjectCgId: 'cg:1', predicate: 'HAS_PROPERTY' }),
      ];
      const cxOutput = createCxOutput('artifact-1', entities, statements);
      const input: MxStageInput = { artifactId: 'artifact-1', cxOutput };
  ...
  describe('canonical semantics', () => {
    it('creates TRIGGERS statements with event as subject, transition as object', async () => {
      ...
      const entities = [
        createEntity({ cgId: 'cg:state-a', name: 'State A', type: 'state' }),
        createEntity({ cgId: 'cg:state-b', name: 'State B', type: 'state' }),
        createEntity({ cgId: 'cg:event-1', name: 'Approval Event', type: 'event' }),
      ];
      const statements = [
        createStatement({
          id: 'stmt:trans',
    ...
    it('synthesizes HAS_STATE statements when single resource inferred', async () => {
      ...
      const hasStateStmts = allStatements.filter(s => s.predicate === 'HAS_STATE');
    ...
    it('lowers TRANSITIONS_TO to canonical FROM_STATE/TO_STATE bindings', async () => {
      ...
      const transitionsToStmts = allStatements.filter(s => s.predicate === 'TRANSITIONS_TO');
      expect(transitionsToStmts.length).toBe(0);

      // Should have FROM_STATE and TO_STATE instead
      const fromStateStmts = allStatements.filter(s => s.predicate === 'FROM_STATE');
      const toStateStmts = allStatements.filter(s => s.predicate === 'TO_STATE');
...
describe('PX Stage', () => {
  ...
  describe('runPxStage', () => {
    ...
    it('filters entities below confidence threshold', async () => {
      const entities = [
        createEntity({ cgId: 'cg:1', name: 'High Confidence', confidence: 0.9 }),
    ...
    it('records filter decisions', async () => {
      ...
      const lowConfDecision = result.filterDecisions.find(d => d.id === 'cg:2');
