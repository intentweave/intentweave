...
describe("Phase 1 E2E Integration Tests", () => {
  describe("Task 1.7.1: MockLLMProvider E2E Flow", () => {
    ...
    it("should extract entities from markdown via ExtractionProvider", async () => {
      ...
      const chunks = createChunksFromContent(
        SAMPLE_MARKDOWN,
        "auth-system.md",
        {
          maxChunkSize: 2000,
      ...
      const result = await extractionProvider.extract(
        chunks,
        {
          kinds: ["role", "state", "action"],
          predicates: ["TRANSITIONS_TO", "ROLE_CAN"],
        },
    ...
    it("should store extracted entities in MemoryStore", async () => {
      ...
      const readSnapshot = await memoryStore.readSnapshot(
        "auth-artifact",
        "RX",
      );
  ...
  describe("Task 1.7.2: OpenAI Integration Test (Optional)", () => {
    ...
    it.skipIf(!hasApiKey)(
      "should extract entities using OpenAI (requires OPENAI_API_KEY)",
      async () => {
        // This test only runs when OPENAI_API_KEY is set
        const { createOpenAILLMProvider } =
          await import("../providers/llm/openai.js");
...
`;
        ...
        expect(
          entityNames.some((n) => n.includes("admin") || n.includes("user")),
        ).toBe(true);
      },
    );
  });
