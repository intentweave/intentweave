 ...
 */
function createEntity(type: EntityType, name: string, confidence = 0.8): Entity {
  return {
...
function resolveReference(
  ...
  const allowedTypes = role === 'subject'
    ...
    : getAllowedObjectTypes(predicate).filter(t => t !== 'null') as EntityType[];
...
describe('Normalization', () => {
  ...
  it('should handle mixed formats', () => {
    expect(normalizeReference('User_Account_Lifecycle')).toBe('user-account-lifecycle');
    expect(normalizeReference('userAccountLifecycle')).toBe('user-account-lifecycle');
  });
...
describe('Shape Constraints', () => {
  ...
  it('should have TRANSITIONS_TO signature: state → state', () => {
    const shape = SHAPE_CONSTRAINTS.find(s => s.predicate === 'TRANSITIONS_TO');
    expect(shape).toBeDefined();
...
describe('Reference Resolution', () => {
  describe('ROLE_CAN(admin, read)', () => {
    it('should resolve admin → role/admin and read → action/read', () => {
      ...
      const subjectRef = resolveReference('admin', 'ROLE_CAN', 'subject', entities);
      expect(subjectRef.resolved).toBe(true);
      expect(subjectRef.cgId).toContain('role/admin');
      expect(subjectRef.type).toBe('role');

      const objectRef = resolveReference('read', 'ROLE_CAN', 'object', entities);
  ...
  describe('HAS_STATE(document, submitted)', () => {
    it('should resolve document → resource/document and submitted → state/submitted', () => {
      ...
      const subjectRef = resolveReference('document', 'HAS_STATE', 'subject', entities);
      expect(subjectRef.resolved).toBe(true);
      expect(subjectRef.cgId).toContain('resource/document');
      expect(subjectRef.type).toBe('resource');

      const objectRef = resolveReference('submitted', 'HAS_STATE', 'object', entities);
      expect(objectRef.resolved).toBe(true);
  ...
  describe('Collision case: active as both state AND action', () => {
    it('should resolve based on predicate signature when ambiguous', () => {
      ...
      const stateRef = resolveReference('active', 'TRANSITIONS_TO', 'object', entities);
      ...
      const actionRef = resolveReference('active', 'ROLE_CAN', 'object', entities);
  ...
  describe('Unresolved references', () => {
    it('should return unresolved when no entity matches by name', () => {
      ...
      const ref = resolveReference('nonexistent', 'ROLE_CAN', 'subject', entities);
    ...
    it('should return unresolved with fallback type for unknown predicates', () => {
      const entities = [
        createEntity('resource', 'doc'),
      ];

      const ref = resolveReference('doc', 'UNKNOWN_PREDICATE', 'subject', entities);
  ...
  describe('Normalization-based resolution', () => {
    it('should resolve camelCase "UserDeactivated" to slug "user-deactivated"', () => {
      ...
      const ref = resolveReference('UserDeactivated', 'HAS_STATE', 'object', entities);
      expect(ref.resolved).toBe(true);
    ...
    it('should resolve underscored "user_deactivated" to slug "user-deactivated"', () => {
      const entities = [
        createEntity('state', 'user-deactivated'),
      ];

      const ref = resolveReference('user_deactivated', 'HAS_STATE', 'object', entities);
      expect(ref.resolved).toBe(true);
      expect(ref.matchedEntity?.name).toBe('user-deactivated');
    });
    ...
    it('should resolve entity in slug to camelCase reference', () => {
      ...
      const ref = resolveReference('user-deactivated', 'HAS_STATE', 'object', entities);
      expect(ref.resolved).toBe(true);
