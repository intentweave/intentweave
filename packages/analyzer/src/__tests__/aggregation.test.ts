 ...
 */
function createEntity(overrides: Partial<Entity> & { name: string; type: string }): Entity {
  return {
...
describe('Aggregation Integration', () => {
  ...
  describe('runAggregation', () => {
    it('aggregates entities from multiple artifacts', async () => {
      ...
      const intentPx = createPxOutput('prompt.md', 'intent', intentEntities, []);
      ...
      const intentCount = result.entities.filter(e => e.artifactId === 'prompt.md').length;
      const specCount = result.entities.filter(e => e.artifactId === 'spec.md').length;
    ...
    it('generates LX proposals for similar entities across artifacts', async () => {
      ...
      const intentEntities = [
        ...
        createEntity({ name: 'suspended', type: 'state', cgId: 'intent-suspended' }),
      ];

      const specEntities = [
        createEntity({ name: 'active', type: 'state', cgId: 'spec-active' }),
        createEntity({ name: 'suspended', type: 'state', cgId: 'spec-suspended' }),
      ];
      ...
      const result = await runAggregation(input, ctx, { generateLxProposals: true });
    ...
    it('calculates coverage metrics', async () => {
      ...
      const result = await runAggregation(input, ctx, { calculateCoverage: true });
    ...
    it('generates rich coverage report when enabled', async () => {
      ...
      expect(result.coverageReport?.$schema).toBe('intentweave://schemas/coverage-report/v1');
    });

    it('generates rich validation output when enabled', async () => {
      const entities = [
        createEntity({ name: 'test', type: 'state' }),
      ];
      ...
      expect(result.validationOutput?.executionTimeMs).toBeGreaterThanOrEqual(0);
    });
  });
...
describe('Aggregation with multiple roles', () => {
  ...
  it('tracks entities by artifact role', async () => {
    ...
    const codeEntities = [createEntity({ name: 'loginFunction', type: 'function' })];
    const testEntities = [createEntity({ name: 'loginTest', type: 'testCase' })];
  ...
  it('generates cross-role link proposals', async () => {
    ...
    const intentEntities = [
      createEntity({ name: 'authenticate', type: 'action', cgId: 'intent-authenticate' }),
    ];
    const specEntities = [
      createEntity({ name: 'authenticate', type: 'action', cgId: 'spec-authenticate' }),
    ];
    ...
    const authProposals = result.lxProposals.filter(
      p => p.sourceCgId.includes('authenticate') || p.targetCgId.includes('authenticate')
    );
