  ...
  describe('MemoryStore', () => {
    ...
    describe('chunk operations', () => {
      const testChunks: Chunk[] = [
        { id: 'chunk-1', artifactId: 'art-1', content: 'First chunk', start: 0, end: 100, index: 0 },
        { id: 'chunk-2', artifactId: 'art-1', content: 'Second chunk', start: 101, end: 200, index: 1 },
      ];
    ...
    describe('run operations', () => {
      ...
      it('lists runs', async () => {
        await store.saveRunMeta({ runId: 'run-1', workspaceId: 'ws', startedAt: '', status: 'completed' });
        await store.saveRunMeta({ runId: 'run-2', workspaceId: 'ws', startedAt: '', status: 'running' });
      ...
      it('deletes run', async () => {
        await store.saveRunMeta({ runId: 'to-delete', workspaceId: 'ws', startedAt: '', status: 'completed' });
  ...
  describe('FileStore', () => {
    ...
    describe('snapshot persistence', () => {
      ...
      it('writes snapshot to correct file', async () => {
        ...
        const exists = await fs.stat(snapshotPath).then(() => true).catch(() => false);
    ...
    describe('aggregate persistence', () => {
      it('persists aggregates to aggregate directory', async () => {
        ...
        const exists = await fs.stat(aggPath).then(() => true).catch(() => false);
        expect(exists).toBe(true);
      });
    });
  });
