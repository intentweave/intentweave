...
import { createRunMeta, completeRunMeta, failRunMeta } from '../pipeline/context.js';
...
import { runAggregation, type AggregateOutput } from '../pipeline/aggregation.js';
...
import { IncrementalCache, computeStageConfigHash, computeGlobalConfigHash, computePxSetHash } from './cache.js';
import { ArtifactRegistry, computeContentHash, hashContent } from './registry.js';
...
export class IncrementalExecutor {
  ...
  async execute(
    ...
    try {
        ...
        try {
          const output = await this.processArtifact(
            ...
            (stage) => {
              ...
              const progress = (processedArtifacts - 1 + (stageIdx + 1) / totalStages) / plan.totalArtifacts;
              onProgress({
          ...
          ctx.logger.error(`Error processing artifact ${keyString}`, { error: err.message });
      ...
      if (!skipAgg && artifactOutputs.length > 0) {
        ...
        // Cache AGG output
        await this.cacheGlobalOutput('AGG', 'all', aggregateOutput, config, pxOutputs);
  ...
  private async processArtifact(
    ...
    if (recomputeSet.has('IN')) {
      ...
      inOutput = await runInStage(inInput, ctx, maxChunkSize ? { maxChunkSize } : {});
      const ms = Date.now() - startTime;
      
      // Cache the output
      const outputHash = hashContent(JSON.stringify(inOutput));
      outputHashes['IN'] = outputHash;
      await this.cacheStageOutput('IN', keyString, inOutput, artifact, config, {}, outputHash, ms);
    } else {
      stageHits++;
    ...
    if (recomputeSet.has('RX')) {
      ...
      const startTime = Date.now();
      rxOutput = await runRxStage(rxInput, {
      ...
      await this.cacheStageOutput('RX', keyString, rxOutput, artifact, config, { IN: outputHashes['IN']! }, outputHash, ms);
    } else {
      stageHits++;
    ...
    if (recomputeSet.has('CX')) {
      ...
      await this.cacheStageOutput('CX', keyString, cxOutput, artifact, config, { RX: outputHashes['RX']! }, outputHash, ms);
    } else {
      stageHits++;
    ...
    if (recomputeSet.has('MX')) {
      ...
      await this.cacheStageOutput('MX', keyString, mxOutput, artifact, config, { CX: outputHashes['CX']! }, outputHash, ms);
    } else {
      stageHits++;
    ...
    if (recomputeSet.has('PX')) {
      ...
      await this.cacheStageOutput('PX', keyString, pxOutput, artifact, config, { MX: outputHashes['MX']! }, outputHash, ms);
    } else {
      stageHits++;
  ...
  private async writeManifest(manifest: RunManifest): Promise<void> {
    ...
    await fs.writeFile(planPath, JSON.stringify(manifest.plan, null, 2), 'utf-8');
  }
  ...
  async generateReport(result: IncrementalResult): Promise<string> {
    ...
    lines.push('');
    lines.push(`- **Artifacts reused:** ${result.cacheStats.cacheHits}/${result.plan.totalArtifacts}`);
    lines.push(`- **Artifacts recomputed:** ${result.cacheStats.cacheMisses}/${result.plan.totalArtifacts}`);
    ...
    lines.push('## Summary');
    lines.push('');
    lines.push(`- **Total entities:** ${result.meta.summary?.entityCount ?? 0}`);
    lines.push(`- **Total statements:** ${result.meta.summary?.statementCount ?? 0}`);
