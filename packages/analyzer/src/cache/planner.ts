...
import { IncrementalCache, computeStageConfigHash, computePxSetHash, checkCacheValidity } from './cache.js';
...
async function computeGlobalInvalidation(
  ...
  let lxReason: InvalidationReason | undefined = aggInvalid ? 'upstream-changed' : undefined;
...
export async function generateRunPlan(
  ...
  const { config, forceFrom, forceArtifacts = [], includeContext = false, priorSnapshotHash } = options;
  ...
  for (const artifact of artifacts) {
    ...
    const plan: ArtifactPlan = {
      ...
      reason: invalidation.recomputeFrom ? invalidation.stages[invalidation.recomputeFrom].reason : undefined,
      details: invalidation.recomputeFrom ? invalidation.stages[invalidation.recomputeFrom].details : undefined,
    };
...
export function formatRunPlan(plan: RunPlan): string {
  ...
  lines.push('═══════════════════════════════════════════════════════════════');
  lines.push('                      INCREMENTAL RUN PLAN                      ');
  lines.push('═══════════════════════════════════════════════════════════════');
  lines.push('');
  ...
  lines.push(`  AGG: ${plan.global.aggInvalid ? `recompute (${plan.global.aggReason})` : 'reuse'}`);
  lines.push(`  LX:  ${plan.global.lxInvalid ? `recompute (${plan.global.lxReason})` : 'reuse'}`);
  lines.push('');
