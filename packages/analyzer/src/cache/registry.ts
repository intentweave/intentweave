...
export async function discoverChatArtifacts(
  ...
  try {
    ...
    console.warn(
      `Warning: Could not read chat turns from ${chatTurnsPath}:`,
      error,
    );
  }
    ...
    for (const sourceEntry of sources) {
      ...
      for (const filename of sessionFiles) {
        ...
        try {
          ...
          console.warn(
            `Warning: Could not read transcript ${sessionPath}:`,
            error,
          );
        }
      }
...
export class ArtifactRegistry {
  ...
  async discover(options: Omit<DiscoveryOptions, "basePath">): Promise<void> {
    ...
    if (options.includeTranscripts !== false) {
      const transcriptArtifacts = await discoverTranscriptArtifacts(
        this.basePath,
        {
