...
function seedFixtures(db: Database.Database) {
  ...
  const symbols = [
    ["impl:src/auth/service.ts#class:AuthService", "AuthService", "class", null, "class AuthService", "src/auth/service.ts", 10, 100, "exported", "Handles authentication"],
    ["impl:src/auth/service.ts#method:AuthService.validateUser", "validateUser", "method", "AuthService", "validateUser(credentials: Credentials): Promise<User>", "src/auth/service.ts", 25, 50, "exported", "Validates user credentials"],
    ["impl:src/auth/jwt.ts#function:signToken", "signToken", "function", null, "signToken(payload: JwtPayload): string", "src/auth/jwt.ts", 5, 20, "exported", "Signs JWT token"],
    ["impl:src/auth/jwt.ts#function:verifyToken", "verifyToken", "function", null, "verifyToken(token: string): JwtPayload", "src/auth/jwt.ts", 25, 40, "exported", "Verifies JWT token"],
    ["impl:src/middleware/rate.ts#class:RateLimiter", "RateLimiter", "class", null, "class RateLimiter", "src/middleware/rate.ts", 1, 60, "exported", "Rate limiting middleware"],
    ["impl:src/db/pool.ts#class:DatabasePool", "DatabasePool", "class", null, "class DatabasePool", "src/db/pool.ts", 1, 80, "exported", "Database connection pool"],
    ["impl:src/db/pool.ts#method:DatabasePool.getConnection", "getConnection", "method", "DatabasePool", "getConnection(): Connection", "src/db/pool.ts", 30, 50, "exported", null],
    ["impl:src/utils/logger.ts#function:createLogger", "createLogger", "function", null, "createLogger(name: string): Logger", "src/utils/logger.ts", 1, 15, "exported", null],
    ["impl:src/config.ts#function:loadConfig", "loadConfig", "function", null, "loadConfig(): AppConfig", "src/config.ts", 1, 30, "internal", "Loads app configuration"],
  ];
  ...
  const annotations = [
    ["docs/auth.md", 10, "AuthService", "impl:src/auth/service.ts#class:AuthService", 1.0, "heading", null, 0.7],
    ["docs/auth.md", 25, "validateUser", "impl:src/auth/service.ts#method:AuthService.validateUser", 0.95, "code-span", null, 0.8],
    ["docs/auth.md", 30, "signToken", "impl:src/auth/jwt.ts#function:signToken", 0.9, "code-span", null, 0.75],
    ["docs/auth.md", 47, "AuthService", "impl:src/auth/service.ts#class:AuthService", 0.95, "bold", "decision", 0.7],
    ["docs/api.md", 15, "RateLimiter", "impl:src/middleware/rate.ts#class:RateLimiter", 0.85, "heading", null, 0.6],
    ["docs/api.md", 50, "AuthService", "impl:src/auth/service.ts#class:AuthService", 0.8, "code-span", null, 0.7],
    ["docs/api.md", 112, "validateUser", "impl:src/auth/service.ts#method:AuthService.validateUser", 0.88, "code-span", null, 0.8],
    ["docs/database.md", 5, "DatabasePool", "impl:src/db/pool.ts#class:DatabasePool", 1.0, "heading", null, 0.65],
    ["docs/database.md", 20, "getConnection", "impl:src/db/pool.ts#method:DatabasePool.getConnection", 0.7, "code-span", null, 0.5],
    // Ungrounded mentions
  ...
  const coocs = [
    ["AuthService", "validateUser", 5, 0.85, "doc_cooc", '["docs/auth.md","docs/api.md"]'],
    ["AuthService", "signToken", 3, 0.72, "doc_cooc", '["docs/auth.md"]'],
    ["AuthService", "RateLimiter", 2, 0.45, "doc_cooc", '["docs/api.md"]'],
    ["DatabasePool", "getConnection", 2, 0.6, "doc_cooc", '["docs/database.md"]'],
  ...
  const cochanges = [
    ["src/auth/service.ts", "src/auth/jwt.ts", 15, 0.68, 0.9, '["abc123","def456"]'],
    ["src/auth/service.ts", "src/middleware/rate.ts", 5, 0.31, 0.5, '["abc123"]'],
...
describe("connections", () => {
  ...
  it("finds co-change connections for annotated entities", () => {
    ...
    const names = result.connections.map((c) => c.name);
    expect(names.some((n) => n.includes("jwt") || n.includes("rate"))).toBe(true);
  });
