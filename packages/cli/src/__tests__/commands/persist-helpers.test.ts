...
describe("relKey", () => {
  ...
  it("preserves case", () => {
    expect(relKey("React", "DECIDED_FOR", "TypeScript")).toBe(
      "React|DECIDED_FOR|TypeScript",
    );
  });
...
describe("delta diff computation", () => {
  ...
  function computeEntityDiff(
    ...
    existing: Map<
      string,
      { name: string; type: string; confidence: number; aliases: string[] }
    >,
  ) {
    const toAdd: Entity[] = [];
    ...
    for (const [canonId, entity] of incoming) {
      ...
      if (!ex) {
        ...
        JSON.stringify([...entity.aliases].sort()) !==
          JSON.stringify([...ex.aliases].sort())
      ) {
  ...
  it("detects updated entities (confidence change)", () => {
    ...
    const existing = new Map([
      [
        "react",
        { name: "React", type: "technology", confidence: 0.85, aliases: [] },
      ],
    ]);
  ...
  it("detects updated entities (name change)", () => {
    ...
    const existing = new Map([
      [
        "react",
        { name: "React", type: "technology", confidence: 0.95, aliases: [] },
      ],
    ]);
  ...
  it("handles mixed add/update/remove/unchanged", () => {
    ...
    const existing = new Map([
      ...
      [
        "vue",
        { name: "Vue", type: "technology", confidence: 0.9, aliases: [] },
      ], // will be updated
      [
        "angular",
        { name: "Angular", type: "technology", confidence: 0.85, aliases: [] },
      ], // will be removed
    ]);
...
describe("relationship diff computation", () => {
  ...
  it("identifies stale relationships to remove", () => {
    const incoming = new Map([["react|USES|jsx", { predicate: "USES" }]]);
    const existing = new Map([
      ["react|USES|jsx", { predicate: "USES", confidence: 0.9 }],
...
describe("orphan relationship filtering", () => {
  it("skips triples where subject entity does not exist", () => {
    ...
    const valid = triples.filter(
      (t) => entities.has(t.subjectCanonId) && entities.has(t.objectCanonId),
    );
