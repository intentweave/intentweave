...
describe("getPricing", () => {
  it("returns exact match for known models", () => {
    ...
    expect(p.completionPer1M).toBe(0.6);
  });
...
describe("computeCost", () => {
  it("computes cost for 1M prompt tokens", () => {
    const cost = computeCost(1_000_000, 0, {
      promptPer1M: 2.5,
  ...
  it("computes cost for 1M completion tokens", () => {
    const cost = computeCost(0, 1_000_000, {
      promptPer1M: 2.5,
      completionPer1M: 10.0,
    });
  ...
  it("computes combined cost", () => {
    const cost = computeCost(500_000, 200_000, {
      promptPer1M: 2.5,
      completionPer1M: 10.0,
    });
  ...
  it("returns 0 for zero tokens", () => {
    expect(computeCost(0, 0, { promptPer1M: 2.5, completionPer1M: 10.0 })).toBe(
      0,
    );
  });
});
...
describe("estimateTokenCost", () => {
  ...
  it("KX calls scale with estimated triples", () => {
    ...
    expect(est.estimatedKxCalls).toBe(
      Math.ceil(
        (10 * ESTIMATION_CONSTANTS.TRIPLES_PER_CHUNK) /
          ESTIMATION_CONSTANTS.KX_BATCH_SIZE,
      ),
    );
  });
});
