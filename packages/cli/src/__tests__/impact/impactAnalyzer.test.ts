...
describe('formatImpactMarkdown', () => {
    ...
    const result = createImpactResult({
      directEntities: [
        createImpactEntity({
          ...
          via: 'direct',
          depth: 0,
          codeRef: { filePath: 'package.json', kind: 'package-dep', strategy: 'dep' },
        }),
      ],
      stats: { filesAnalyzed: 1, directCount: 1, rippleCount: 0, totalRelationships: 0, decisionCount: 0, riskCount: 0 },
    });
    const md = formatImpactMarkdown(result);
  ...
  it('renders ripple impact grouped by hop depth', () => {
    const result = createImpactResult({
      ...
      stats: { filesAnalyzed: 1, directCount: 0, rippleCount: 3, totalRelationships: 0, decisionCount: 0, riskCount: 0 },
    });
    const md = formatImpactMarkdown(result);
  ...
  it('renders decision trail with DECIDED_FOR / DECIDED_AGAINST', () => {
    const result = createImpactResult({
      ...
      stats: { filesAnalyzed: 1, directCount: 0, rippleCount: 0, totalRelationships: 0, decisionCount: 2, riskCount: 0 },
    });
    const md = formatImpactMarkdown(result);
  ...
  it('renders risks and blockers', () => {
    const result = createImpactResult({
      ...
      stats: { filesAnalyzed: 1, directCount: 0, rippleCount: 0, totalRelationships: 0, decisionCount: 0, riskCount: 2 },
    });
    const md = formatImpactMarkdown(result);
  ...
  it('renders key relationships grouped by predicate', () => {
    const result = createImpactResult({
      relationships: [
        createImpactRelationship({ predicate: 'DEPENDS_ON', sourceName: 'A', targetName: 'B' }),
        createImpactRelationship({ predicate: 'DEPENDS_ON', sourceName: 'C', targetName: 'D' }),
        createImpactRelationship({ predicate: 'USES', sourceName: 'E', targetName: 'F' }),
      ],
      stats: { filesAnalyzed: 1, directCount: 0, rippleCount: 0, totalRelationships: 3, decisionCount: 0, riskCount: 0 },
    });
    const md = formatImpactMarkdown(result);
  ...
  it('excludes DECIDED_FOR/AGAINST/RISKS/BLOCKS from key relationships', () => {
    const result = createImpactResult({
      relationships: [
        createImpactRelationship({ predicate: 'DECIDED_FOR', sourceName: 'X', targetName: 'Y' }),
        createImpactRelationship({ predicate: 'RISKS', sourceName: 'X', targetName: 'Y' }),
      ],
      stats: { filesAnalyzed: 1, directCount: 0, rippleCount: 0, totalRelationships: 2, decisionCount: 0, riskCount: 0 },
    });
    const md = formatImpactMarkdown(result);
  ...
  it('truncates key relationships to top 10 per predicate', () => {
    const rels = Array.from({ length: 15 }, (_, i) =>
      createImpactRelationship({ predicate: 'USES', sourceName: `S${i}`, targetName: `T${i}` }),
    );
    const result = createImpactResult({
      relationships: rels,
      stats: { filesAnalyzed: 1, directCount: 0, rippleCount: 0, totalRelationships: 15, decisionCount: 0, riskCount: 0 },
    });
    const md = formatImpactMarkdown(result);
...
describe('analyzeImpact', () => {
  ...
  it('finds direct entities via CodeRef queries', async () => {
    const runner = createSequentialMockRunner([
      ...
      [
        { name: 'React', type: 'technology', confidence: 0.99, filePath: 'package.json', kind: 'package-dep', strategy: 'dep' },
        { name: 'TypeScript', type: 'technology', confidence: 0.95, filePath: 'tsconfig.json', kind: 'file', strategy: 'path' },
      ],
  ...
  it('expands ripple entities and classifies decisions/risks', async () => {
    const runner = createSequentialMockRunner([
      ...
      // Step 3: Relationships in subgraph
      [
        { sName: 'frontend', sType: 'component', predicate: 'USES', tName: 'React', tType: 'technology', confidence: 0.95 },
        { sName: 'architecture', sType: 'decision', predicate: 'DECIDED_FOR', tName: 'React', tType: 'technology', confidence: 0.9 },
        { sName: 'complexity', sType: 'risk', predicate: 'RISKS', tName: 'frontend', tType: 'component', confidence: 0.7 },
      ],
