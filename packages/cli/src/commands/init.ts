...
export const initCommand = new Command('init')
  ...
  .action(async (directory: string, options) => {
    ...
    try {
      ...
      if (!force) {
        console.error(chalk.red('Workspace already initialized. Use --force to overwrite.'));
        process.exit(1);
      }
    } catch {
    ...
    // Create config
    const config = createWorkspaceConfig(workspaceId, workspaceName, absoluteDir);
    ...
    console.log(`  ${chalk.blue(`${CLI_NAME} analyze`)} - Analyze files in this workspace`);
    console.log(`  ${chalk.blue(`${CLI_NAME} status`)} - Show workspace status`);
  });
