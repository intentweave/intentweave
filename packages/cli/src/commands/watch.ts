...
export const watchCommand = new Command("watch")
  ...
  .action(async (files: string[], options) => {
    ...
    const watchPaths =
      files.length > 0
        ? files.map((f) => (path.isAbsolute(f) ? f : path.join(cwd, f)))
        : [cwd];
    ...
    console.log(
      `  Provider:    ${providerName}${providerName === "openai" ? ` (${modelName})` : ""}`,
    );
    ...
    async function runCycle(changedAbsPaths: string[]) {
      ...
      try {
        ...
        const openResults: OpenTrackResult[] = await runOpenTrackBatch(
          artifacts,
          ctx,
          {
            llmProvider: llmProvider as any,
        ...
        if (shouldPersist && openResults.length > 0) {
          try {
            ...
            if (d) {
              console.log(
                `  ${chalk.cyan("Neo4j:")} +${d.entities.added} ~${d.entities.updated} -${d.entities.removed} entities, +${d.relationships.added} -${d.relationships.removed} rels`,
              );
            } else {
              console.log(
                `  ${chalk.cyan("Neo4j:")} ${persistResult.canonEntitiesWritten} entities, ${persistResult.canonRelationshipsWritten} rels`,
              );
            }
          } catch (persistErr) {
            console.error(
              chalk.yellow(
                `  ⚠ Persist failed: ${(persistErr as Error).message}`,
              ),
            );
          }
        }
        ...
        const totalEntities = openResults.reduce(
        ...
        const cacheNote =
          fxCached > 0 || kxCached > 0
            ? chalk.green(
                ` (${fxCached}/${openResults.length} FX cached, ${kxCached}/${openResults.length} KX cached)`,
              )
            : "";

        // Token usage
        const usages = openResults
          .map((r) => r.tokenUsage)
          .filter((u): u is TokenUsage => u != null);
        const costNote =
          usages.length > 0
            ? ` ${formatCost(sumTokenUsage(...usages).costUsd)}`
            : "";
        ...
        console.error(
          `${ts()} ${chalk.red("✗")} Cycle failed: ${(err as Error).message}`,
        );
        if (verbose) {
