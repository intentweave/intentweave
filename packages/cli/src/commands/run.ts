...
async function loadCachedFxOutputs(
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      throw new Error(
        `FX results not found at: ${fxPath}\n` +
          `  Hint: run with --track open first, then use --from-fx <run-id>`,
      );
    }
    throw err;
...
export const runCommand = new Command("run")
  ...
  .option(
    "--provider <name>",
    "LLM provider to use (smart-mock|openai)",
    "smart-mock",
  )
  .option(
    "--model <name>",
    "Model to use (for OpenAI: gpt-5-mini|gpt-4o|gpt-4o-mini)",
    "gpt-5-mini",
  )
  .option(
    "--api-key <key>",
    "API key override (uses OPENAI_API_KEY env var by default)",
  )
  .option(
    "--timeout <ms>",
  ...
  .option(
    "--plan",
    "Show execution plan without running (requires --incremental)",
  )
  .option("--force", "Force full recomputation (with --incremental)")
  .option(
    "--force-from <stage>",
    "Force recomputation from stage (IN|RX|CX|MX|PX)",
  )
  // Transcript options
  .option("--no-transcripts", "Exclude transcript sessions from discovery")
  .option(
    "--transcript-limit <n>",
    "Limit number of transcripts to process",
    parseInt,
  )
  ...
  .option(
    "--include-ignored",
    "Include files normally ignored (node_modules, dist, etc.)",
  )
  .option(
    "--max-artifacts <n>",
    "Maximum number of artifacts to process",
    parseInt,
  )
  // Role override
  .option(
    "-r, --role <role>",
    "Override artifact role (intent|spec|implementation|test|config)",
  )
  // Track selection
  .option(
    "-t, --track <track>",
    "Pipeline track: main (schema-constrained), open (schema-free), or both",
    "main",
  )
  .option(
    "--from-fx <source>",
    "Skip FX, load cached FX output and run only KX. Value: run ID or path to fx-results.json",
  )
  // Neo4j persistence
  .option(
    "--persist",
    "Persist open track (KX) results to Neo4j (requires NEO4J_PASSWORD env var)",
  )
  .option(
    "--neo4j-uri <uri>",
  ...
  .action(async (files: string[], options) => {
    ...
    if (!["main", "open", "both"].includes(track)) {
      console.error(
        chalk.red(`Invalid track: ${track}. Must be one of: main, open, both`),
      );
      process.exit(1);
    }
    ...
    if (!transcriptsOnly) {
      ...
      if (maxArtifacts && filteredFiles.length > maxArtifacts) {
        console.log(
          chalk.yellow(
            `Limiting to ${maxArtifacts} artifacts (found ${filteredFiles.length})`,
          ),
        );
    ...
    if (filteredFiles.length > MAX_RECOMMENDED) {
      ...
      console.log(
        chalk.yellow(`   This may take a while and use significant memory.`),
      );
      console.log(
        chalk.yellow(
          `   Consider using --max-artifacts ${MAX_RECOMMENDED} to limit scope.\n`,
        ),
      );
    }
    ...
    if (dryRun) {
      ...
      if (track === "open" || track === "both") {
        const model =
          modelName ??
          (providerName === "openai" ? "gpt-5-mini" : "smart-mock");
    ...
    console.log(`Profile: ${profile.name}`);
    console.log(
      `Track: ${track === "main" ? chalk.blue(track) : track === "open" ? chalk.magenta(track) : chalk.yellow(track)}`,
    );
    if (incrementalMode) {
    ...
    if (providerName === "openai") {
      ...
      if (!apiKey) {
        ...
        console.log(
          chalk.yellow(
            "Or use default provider: iw run (uses smart-mock, no API key needed)",
          ),
        );
        process.exit(1);
      }
      ...
      if (verbose) {
        console.log(
          chalk.blue(
            `Using OpenAI provider with model: ${modelName}${timeoutMs ? `, timeout: ${timeoutMs}ms` : ""}`,
          ),
        );
      }
    } else {
      ...
      if (verbose) {
        console.log(
          chalk.blue(
            "Using SmartMock provider (deterministic, no API key needed)",
          ),
        );
      }
    }
    ...
    const onProgress = (progress: PipelineProgress) => {
      if (verbose) {
        ...
        console.log(
          `    ${progress.artifactId} ${formatProgress(progress.progress)}`,
        );
      } else {
        process.stdout.write(
          `\r  Processing: ${formatProgress(progress.progress)} [${progress.stage}]`,
        );
      }
    };
    ...
    try {
      ...
      if (track === "open" || track === "both") {
        ...
        if (fromFxSource) {
          console.log(
            chalk.magenta("Running KX-only from cached FX output..."),
          );
          const fxOutputs = await loadCachedFxOutputs(
            fromFxSource,
            cwd,
            outputDir,
          );
          console.log(
            chalk.magenta(
              `  Loaded ${fxOutputs.length} artifact(s) with ${fxOutputs.reduce((s, f) => s + f.triples.length, 0)} raw triples`,
            ),
          );

          openResults = [];
          for (const fxOut of fxOutputs) {
            const kxResult = await runKxFromFxOutput(fxOut, ctx, {
              llmProvider,
            });
          ...
          const openTrackCache = incrementalMode
            ? new OpenTrackCache(cwd)
            : undefined;
          if (openTrackCache) {
            await openTrackCache.init();
            console.log(
              chalk.magenta("Running open track (IN → FX → KX) — incremental…"),
            );
          } else {
      ...
      if (openResults && openResults.length > 1) {
        ...
        console.log(
          `  Entities: ${gxOutput.meta.inputEntityCount} → ${gxOutput.meta.outputEntityCount} (${gxOutput.meta.exactMerges} exact, ${gxOutput.meta.fuzzyMerges} fuzzy merges)`,
        );
        console.log(
          `  Triples:  ${gxOutput.meta.inputTripleCount} → ${gxOutput.meta.outputTripleCount} (${gxOutput.meta.inputTripleCount - gxOutput.meta.outputTripleCount} deduped)`,
        );
      ...
      if (shouldPersist && openResults && openResults.length > 0) {
        try {
          ...
          if (gxOutput) {
            ...
            console.log(
              chalk.cyan("Persisting open track results to Neo4j..."),
            );
          ...
          console.log(
            chalk.green(
              `✓ Neo4j persistence complete (${(persistResult.durationMs / 1000).toFixed(1)}s)`,
            ),
          );
          if (persistResult.delta) {
            const d = persistResult.delta;
            console.log(
              `  Entities:      +${d.entities.added} ~${d.entities.updated} -${d.entities.removed} =${d.entities.unchanged}`,
            );
            console.log(
              `  Relationships: +${d.relationships.added} -${d.relationships.removed} =${d.relationships.unchanged}`,
            );
            console.log(
              `  Raw triples:   +${d.rawTriples.added} -${d.rawTriples.removed} =${d.rawTriples.unchanged}`,
            );
          } else {
            console.log(
              `  Canon entities: ${persistResult.canonEntitiesWritten}`,
            );
            console.log(
              `  Canon relationships: ${persistResult.canonRelationshipsWritten}`,
            );
          ...
          console.log(
            chalk.dim(
              "  Results are still saved to disk. You can persist later with --from-fx.",
            ),
          );
        }
      }
      ...
      if (result?.aggregate && !skipAgg) {
        ...
        if (result.aggregate.coverage) {
          ...
          await fs.writeFile(
            coveragePath,
            JSON.stringify(result.aggregate.coverage, null, 2),
          );
        }
        ...
        if (result.aggregate.findings) {
          ...
          await fs.writeFile(
            findingsPath,
            JSON.stringify(result.aggregate.findings, null, 2),
          );
        }
        ...
        try {
          ...
          if (verbose) {
            console.error(
              chalk.yellow(
                `Warning: Could not generate bundle: ${(bundleErr as Error).message}`,
              ),
            );
          }
        }
      ...
      if (openResults && openResults.length > 0) {
        ...
        const totalLatency = openResults.reduce(
          (s, r) => s + r.meta.totalLatencyMs,
          0,
        );
        ...
        if (artifactUsages.length > 0) {
          ...
          console.log(
            `  Tokens: ${formatTokens(totalUsage.totalTokens)} (${formatTokens(totalUsage.promptTokens)} prompt + ${formatTokens(totalUsage.completionTokens)} completion)`,
          );
          console.log(`  Est. cost: ${formatCost(totalUsage.costUsd)}`);
        }

        // Show incremental cache stats
        const fxCached = openResults.filter(
          (r) => r.meta.stages.FX.cached,
        ).length;
        const kxCached = openResults.filter(
          (r) => r.meta.stages.KX.cached,
        ).length;
        if (fxCached > 0 || kxCached > 0) {
          console.log(
            `  Cache: ${chalk.green(`${fxCached}/${openResults.length} FX cached`)}, ${chalk.green(`${kxCached}/${openResults.length} KX cached`)}`,
          );
        }
        ...
        if (verbose) {
          ...
          for (const r of openResults) {
            console.log(
              `    ${chalk.dim(r.artifactId)}: ${r.kx.rawTriples.length} raw → ${r.kx.canonTriples.length} canon triples, ${r.kx.canonEntities.length} entities`,
            );
          }
          ...
          if (allEntities.length > 0) {
            ...
            for (const e of sorted) {
              console.log(
                `    ${chalk.cyan(e.name)} (${e.type}) [${(e.confidence * 100).toFixed(0)}%]`,
              );
            }
          }
          ...
          if (allTriples.length > 0) {
            ...
            for (const t of sample) {
              console.log(
                `    ${chalk.cyan(t.subjectCanonId)} ${chalk.yellow(`─[${t.predicate}]→`)} ${chalk.cyan(t.objectCanonId)}`,
              );
            }
            if (allTriples.length > 10) {
              console.log(
                chalk.dim(`    ... and ${allTriples.length - 10} more`),
              );
            }
          }
        }
        ...
        if (gxOutput) {
          ...
          console.log(
            `  Entities: ${gxOutput.meta.inputEntityCount} → ${gxOutput.meta.outputEntityCount}`,
          );
          console.log(
            `  Triples:  ${gxOutput.meta.inputTripleCount} → ${gxOutput.meta.outputTripleCount}`,
          );
          console.log(
            `  Exact merges: ${gxOutput.meta.exactMerges}, Fuzzy merges: ${gxOutput.meta.fuzzyMerges}`,
          );

          if (
            verbose &&
            gxOutput.merges.filter((m) => m.method === "fuzzy").length > 0
          ) {
            console.log(chalk.dim("  Fuzzy merges:"));
            for (const m of gxOutput.merges.filter(
              (m) => m.method === "fuzzy",
            )) {
              console.log(
                chalk.dim(
                  `    ${m.mergedIds.join(", ")} → ${m.survivorId} (${((m.similarity ?? 0) * 100).toFixed(0)}%)`,
                ),
              );
            }
          }
      ...
      if (result?.aggregate && !skipAgg) {
        ...
        if (result.aggregate.lxProposals) {
          console.log(
            `  Link proposals: ${result.aggregate.lxProposals.length}`,
          );
        }
        if (result.aggregate.coverage) {
          console.log(
            `  Coverage: ${result.aggregate.coverage.summary.totalConcepts} concepts across ${result.aggregate.coverage.summary.totalArtifacts} artifacts`,
          );
        }
        if (result.aggregate.findings) {
          console.log(
            `  Findings: ${result.aggregate.findings.findings?.length ?? 0}`,
          );
        }
      }
      ...
      console.log("");
      console.log(
        chalk.green(`Output written to: ${outputDir}/runs/${runId}/`),
      );
      if (result) {
        console.log(chalk.dim(`  overview.json - Quick summary`));
        console.log(
          chalk.dim(
            `  bundle/graph.json - Consolidated entities/statements/lx`,
          ),
        );
      }
      if (openResults) {
        console.log(
          chalk.dim(`  open-track/kx-results.json - Raw + canonical triples`),
        );
        if (gxOutput) {
          console.log(
            chalk.dim(
              `  open-track/gx-merged.json - Cross-document merged graph`,
            ),
          );
        }
      }
      ...
      console.log(
        `  ${chalk.blue(`${CLI_NAME} report`)} - Generate analysis report`,
      );
    } catch (error) {
      console.error("");
...
async function runIncremental(
  ...
  // Generate the plan
  const forceFromStage = forceFrom as
    | "IN"
    | "RX"
    | "CX"
    | "MX"
  ...
  try {
    ...
    console.log(
      `  Artifacts reused: ${chalk.green(result.cacheStats.cacheHits.toString())}/${plan.totalArtifacts}`,
    );
    console.log(
      `  Artifacts recomputed: ${chalk.yellow(result.cacheStats.cacheMisses.toString())}/${plan.totalArtifacts}`,
    );
    console.log(
      `  Stages reused: ${chalk.green(result.cacheStats.stageHits.toString())}`,
    );
    console.log(
      `  Stages recomputed: ${chalk.yellow(result.cacheStats.stageMisses.toString())}`,
    );
    console.log(`  Duration: ${duration}s`);
    console.log("");
    ...
    if (result.aggregate && !skipAgg) {
      ...
      if (result.aggregate.coverage) {
        console.log(
          `  Coverage: ${result.aggregate.coverage.summary.totalConcepts} concepts`,
        );
      }
      if (result.aggregate.findings) {
        console.log(
    ...
    console.log(
      chalk.green(`Cache directory: ${executor.getCache().getCacheDir()}`),
    );
    console.log("");
    console.log("Next steps:");
    console.log(`  ${chalk.blue(`${CLI_NAME} status`)} - View run status`);
    ...
    console.log(
      `  ${chalk.blue(`${CLI_NAME} run -i --plan`)} - Preview next run`,
    );
  } catch (error) {
    console.error("");
