...
import { generateBundleV2, type BundleV2Options } from "@intentweave/core";
...
export const weaveCommand = new Command("weave")
  ...
  .option(
    "--explain <canonicalId>",
    "Explain why entities merged into a canonical",
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(
        `  ${chalk.cyan("Canonical entities:")} ${stats.canonicalEntityCount}`,
      );
      console.log(
        `  ${chalk.cyan("Canonical stmts:")}    ${stats.canonicalStatementCount}`,
      );
      ...
      if (stats.rawEntityCount > stats.canonicalEntityCount) {
        const mergeRatio =
          (1 - stats.canonicalEntityCount / stats.rawEntityCount) * 100;
        console.log(
 ...
 */
async function explainCanonical(
  runDir: string,
  canonicalId: string,
): Promise<void> {
  ...
  if (canonical.evidenceIds?.length > 0) {
    console.log(chalk.bold("\nEvidence:"));
    const evidenceRecords = bundle.evidence.filter((e: { id: string }) =>
      canonical.evidenceIds.includes(e.id),
    );
    for (const ev of evidenceRecords.slice(0, 5)) {
      console.log(`  ${chalk.dim("•")} ${ev.ref.uri}`);
      if (ev.excerpt) {
        console.log(
          `    "${ev.excerpt.slice(0, 80)}${ev.excerpt.length > 80 ? "..." : ""}"`,
        );
      }
    }
    ...
    for (const stmt of relatedStmts.slice(0, 5)) {
      ...
      if (isSubject) {
        const obj = stmt.objectCanonicalId
          ? (bundle.weave.entities.find(
              (e: { canonicalId: string }) =>
                e.canonicalId === stmt.objectCanonicalId,
            )?.displayName ?? stmt.objectCanonicalId)
          : `"${stmt.objectLiteral}"`;
        ...
        const subj =
          bundle.weave.entities.find(
            (e: { canonicalId: string }) =>
              e.canonicalId === stmt.subjectCanonicalId,
          )?.displayName ?? stmt.subjectCanonicalId;
