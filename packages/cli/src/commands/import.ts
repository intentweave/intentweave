...
export const importCommand = new Command('import')
  ...
  .option('-a, --adapter <name>', 'Adapter to use (specstory, chatgpt)', 'specstory')
  ...
  .option('--debounce <ms>', 'Debounce delay for watch mode (default: 500ms)', '500')
...
async function runImport(source: string | undefined, options: ImportCommandOptions): Promise<void> {
  const workspaceRoot = options.workspace ?? process.cwd();
  
  // Determine source files
  const sourcePaths = await resolveSourcePaths(source, options.adapter, workspaceRoot);
  ...
  for (const sourcePath of sourcePaths) {
    ...
    try {
      ...
      if (result.newMessages > 0 || options.verbose) {
        printImportResult(result, options.plan ?? false, options.verbose ?? false);
      }
    } catch (error) {
  ...
  console.log('');
  if (options.plan) {
    console.log(`Plan: Would import ${totalNew} new message(s) from ${sourcePaths.length} file(s).`);
  } else {
    console.log(`Imported ${totalNew} new message(s) from ${sourcePaths.length} file(s).`);
 ...
 */
async function discoverSourceFiles(directory: string, adapter: string): Promise<string[]> {
 ...
 * Print import result.
 */
function printImportResult(result: ImportResult, planOnly: boolean, verbose: boolean): void {
  ...
  if (result.newMessages === 0) {
    ...
    console.log(`  ${sessionInfo}: ${action} ${result.newMessages} new message(s)`);
  }
  
  if (verbose && result.roleStats) {
    const { intent, spec, implementation, runlog, meta, unknown } = result.roleStats;
    const parts: string[] = [];
 ...
 */
async function runWatch(source: string | undefined, options: ImportCommandOptions): Promise<void> {
  const workspaceRoot = options.workspace ?? process.cwd();
  ...
  const importFile = async (filePath: string) => {
    ...
    try {
      ...
      if (result.newMessages > 0) {
        ...
        console.log(`[${timestamp}] 📄 ${path.basename(filePath)}: +${result.newMessages} message(s)`);
      } else if (options.verbose) {
        const timestamp = new Date().toLocaleTimeString();
        console.log(`[${timestamp}] 📄 ${path.basename(filePath)}: No new messages`);
      }
    } catch (error) {
      const timestamp = new Date().toLocaleTimeString();
      console.error(`[${timestamp}] ❌ ${path.basename(filePath)}: ${error instanceof Error ? error.message : error}`);
    }
  };
