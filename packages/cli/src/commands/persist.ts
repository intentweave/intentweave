...
export const persistCommand = new Command('persist')
  .description('Persist open track (KX) results to Neo4j (delta mode by default — only writes changes)')
  ...
  .option('--session-id <id>', 'Session ID for Neo4j isolation (default: workspace name)')
  .option('--mode <mode>', 'Persist mode: delta (default, diff-only) | full (legacy, may duplicate)', 'delta')
  .option('--neo4j-uri <uri>', 'Neo4j connection URI (default: bolt://localhost:7687)')
  .option('--neo4j-password <password>', 'Neo4j password (default: NEO4J_PASSWORD env var)')
  ...
  .action(async (runIdArg: string | undefined, options) => {
    ...
    if (filePath) {
      kxResultsPath = path.isAbsolute(filePath) ? filePath : path.join(cwd, filePath);
    } else {
        ...
        if (useLatest) {
          ...
          try {
            ...
            for (const run of sorted) {
              const kxPath = path.join(runsDir, run, 'open-track', 'kx-results.json');
              try {
            ...
            if (!targetRunId) {
              ...
              console.log(chalk.dim(`Run with --track open first: ${CLI_NAME} run --track open --provider openai`));
              process.exit(1);
            }
          } catch {
            console.error(chalk.red(`No IntentWeave workspace found. Run ${CLI_NAME} init first.`));
            process.exit(1);
          }
        } else {
          console.error(chalk.red('Specify a run ID, --latest, or --file <path>'));
          console.log(`\nUsage:`);
          console.log(`  ${chalk.blue(`${CLI_NAME} persist <run-id>`)}`);
          console.log(`  ${chalk.blue(`${CLI_NAME} persist --latest`)}`);
          console.log(`  ${chalk.blue(`${CLI_NAME} persist --file path/to/kx-results.json`)}`);
          process.exit(1);
        }
      }

      kxResultsPath = path.join(runsDir, targetRunId, 'open-track', 'kx-results.json');
    }
    ...
    const totalEntities = kxOutputs.reduce((s, o) => s + (o.canonEntities?.length ?? 0), 0);
    const totalTriples = kxOutputs.reduce((s, o) => s + (o.canonTriples?.length ?? 0), 0);
    const totalRaw = kxOutputs.reduce((s, o) => s + (o.rawTriples?.length ?? 0), 0);
    ...
    console.log(chalk.cyan(`Persisting to Neo4j [${modeLabel}] (session: ${sessionId})...`));

    try {
      const { persistKxToNeo4j } = await import('./persist-neo4j.js');

      const logFn = verbose ? (msg: string) => console.error(chalk.blue(msg)) : undefined;
      ...
      console.log('');
      console.log(chalk.green(`✓ Persisted to Neo4j (${(result.durationMs / 1000).toFixed(1)}s)`));

      if (result.delta) {
        // Delta mode — show detailed diff stats
        const d = result.delta;
        console.log(`  Entities:      +${d.entities.added} added, ~${d.entities.updated} updated, -${d.entities.removed} removed, ${d.entities.unchanged} unchanged`);
        console.log(`  Relationships: +${d.relationships.added} added, -${d.relationships.removed} removed, ${d.relationships.unchanged} unchanged`);
        console.log(`  Raw triples:   +${d.rawTriples.added} added, -${d.rawTriples.removed} removed, ${d.rawTriples.unchanged} unchanged`);
      } else {
        console.log(`  Canon entities: ${result.canonEntitiesWritten}`);
        console.log(`  Canon relationships: ${result.canonRelationshipsWritten}`);
        console.log(`  Raw triples: ${result.rawTriplesWritten}`);
      }
      console.log('');
      console.log('Query examples:');
      console.log(chalk.dim(`  // All entities`));
      console.log(chalk.blue(`  MATCH (n:Canon:Entity { session_id: '${sessionId}' }) RETURN n.name, n.type ORDER BY n.confidence DESC`));
      console.log(chalk.dim(`  // All decisions`));
      console.log(chalk.blue(`  MATCH (a:Canon)-[:DECIDED_FOR]->(b:Canon) RETURN a.name, b.name`));
      console.log(chalk.dim(`  // What enables what`));
      console.log(chalk.blue(`  MATCH (a:Canon)-[:ENABLES]->(b:Canon) RETURN a.name, b.name`));
    } catch (err) {
