...
import { getDefaultProfilePack, loadProfilePack, type ProfilePack } from '@intentweave/profiles';
  ...
  try {
    ...
    for (const artifactId of artifactDirs) {
      ...
      try {
        ...
        const artifactRole = (pxData.artifact?.artifactRole || 'unknown') as ArtifactRole;
    ...
    const lxPath = path.join(iwDir, 'runs', runId, 'aggregate', 'lx.proposals.json');
    try {
...
export const validateCommand = new Command('validate')
  ...
  .option('--severity <level>', 'Minimum severity to show: error, warning, info', 'warning')
  ...
  .action(async (options) => {
    ...
    const { entities, statements, linkProposals } = await loadRunData(iwDir, runId);
    
    console.log(`Found ${entities.length} entities, ${statements.length} statements`);
    ...
    const minSeverityNum = severityOrder[severity as keyof typeof severityOrder] ?? 1;
    ...
    if (filteredFindings.length > 0) {
      ...
      toShow.forEach((f, i) => {
        const severityColor = f.severity === 'error' ? chalk.red 
          : f.severity === 'warning' ? chalk.yellow 
          : chalk.blue;
        console.log(`  ${i + 1}. ${severityColor(`[${f.severity.toUpperCase()}]`)} ${f.message}`);
      ...
      if (!verbose && filteredFindings.length > 10) {
        console.log(`  ... and ${filteredFindings.length - 10} more. Use -v for full list.`);
      }
    }
    
    // Write output
    const outputPath = output || path.join(runDir, 'aggregate', 'findings.json');
