...
function renderTable(result: TriageResult, verbose: boolean): void {
  ...
  for (const c of candidates) {
    ...
    const name =
      c.entityName.length > 33 ? c.entityName.slice(0, 32) + "…" : c.entityName;
...
export const triageCommand = new Command("triage")
  ...
  .action(async (opts) => {
    const { session, max, min, format, output, verbose } = opts;
