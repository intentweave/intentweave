...
export const openCommand = new Command("open")
  ...
  .action(async (sourceKey: string, options: OpenOptions) => {
    ...
    if (!existsSync(transcriptPath)) {
      ...
      console.log("");
      console.log(
        `Run ${chalk.cyan("iw import")} to import transcripts first.`,
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.log(
        `  Role:       ${target.messageRole ?? "undefined"} (${target.roleSource})`,
      );
      if (target.ts) {
      ...
      if (!options.noContext) {
        ...
        for (let i = startIdx; i <= endIdx; i++) {
          ...
          const seqNum = i + 1;
          const speakerColor =
            msg.speaker === "user" ? chalk.blue : chalk.green;
          ...
          if (isTarget) {
            console.log(
              `${prefix} ${chalk.bold(`[${seqNum}]`)} ${speakerColor(msg.speaker)}: ${text}`,
            );
          } else {
            console.log(
              `${prefix} [${seqNum}] ${speakerColor(msg.speaker)}: ${chalk.dim(text)}`,
            );
          }
        }
      ...
      console.log(
        `  iw explain <issueId>             # If this message is evidence`,
      );
      console.log("");
    } catch (error) {
