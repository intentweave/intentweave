...
export const vizCommand = new Command("viz").description(
  "Generate Mermaid visualizations from pipeline data",
);
...
vizCommand
  ...
  .action(async (issueId: string, options: VizIssueOptions) => {
    try {
      const { entities, statements, findings } = await loadGraphData(
        options.run,
      );
      ...
      switch (format) {
        ...
        case "ascii": {
          ...
          if (asciiResult.success) {
            ...
            console.error(
              chalk.yellow(
                `Warning: ASCII rendering failed: ${asciiResult.error}`,
              ),
            );
      ...
      if (options.output) {
        ...
        console.log(
          chalk.green(`✓ Visualization written to ${options.output}`),
        );
      } else {
        console.log(output);
      }
...
vizCommand
  ...
  .option(
    "--ascii",
    "Shorthand for --format dual (ASCII art with Mermaid in details)",
  )
  .option("-o, --output <file>", "Write to file instead of stdout")
  .action(
    async (cgId: string, options: VizEntityOptions & { ascii?: boolean }) => {
      try {
        ...
        const entity = entities.find(
          (e) =>
            e.id === cgId || e.id.includes(cgId) || e.id.endsWith(`/${cgId}`),
        );
        ...
        if (!entity) {
          ...
          const matches = entities
            .filter((e) => e.id.toLowerCase().includes(cgId.toLowerCase()))
            .slice(0, 10);
          if (matches.length > 0) {
...
vizCommand
  ...
  .action(async (options: any) => {
    try {
      ...
      console.log(
        chalk.green(`✓ Generated ${count} visualizations in ${outputDir}`),
      );
    } catch (error) {
...
vizCommand
  ...
  .action(async (options: VizRbacOptions & { ascii?: boolean }) => {
    try {
      ...
      const authStatements = statements.filter(
        (s) =>
      ...
      if (options.output) {
        await fs.writeFile(options.output, output, "utf-8");
        console.log(
          chalk.green(`✓ RBAC visualization written to ${options.output}`),
        );
      } else {
        console.log(output);
      }
...
async function loadGraphData(runId?: string): Promise<GraphData> {
  ...
  try {
    ...
    throw new Error(
      `Graph data not found at ${graphPath}. Run the pipeline first.`,
    );
  }
  ...
  const artifacts = new Map<
    string,
    { id: string; path: string; role: string }
  >();
  ...
  const statements: VizStatement[] = (graphData.statements || []).map(
    (s: any) => ({
      id: s.id,
  ...
  const findings: VizFinding[] = (findingsData.findings || []).map(
    (f: any) => ({
      id: f.id || f.ruleId,
