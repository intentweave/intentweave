...
export const roleCommand = new Command("role").description(
  "Manage message role assignments for transcripts",
);
...
roleCommand
  ...
  .action(async (options) => {
    try {
      ...
      if (options.session) {
        overrides = await getRoleOverridesForSession(
          workspaceRoot,
          options.session,
        );
      } else {
      ...
      console.log(
        `  ${"SOURCE KEY".padEnd(50)} ${"ROLE".padEnd(15)} ${"SET BY".padEnd(8)} REASON`,
      );
    ...
    try {
      ...
      for (const session of sessions) {
        ...
        for (const msg of messages) {
          ...
          stats.byRoleSource[roleSource] =
            (stats.byRoleSource[roleSource] || 0) + 1;
        }
      ...
      for (const stats of allStats) {
        ...
        for (const [role, count] of Object.entries(stats.byRole)) {
          if (count > 0) {
            ...
            console.log(
              `     ${role.padEnd(15)} ${String(count).padStart(4)} (${pct.padStart(5)}%) ${bar}`,
            );
          }
        }
        ...
        for (const [source, count] of Object.entries(stats.byRoleSource)) {
          if (count > 0) {
            const pct = ((count / stats.total) * 100).toFixed(1);
            console.log(
              `     ${source.padEnd(15)} ${String(count).padStart(4)} (${pct.padStart(5)}%)`,
            );
          }
        }
      }
      ...
      if (allStats.length > 1) {
        ...
        for (const [role, count] of Object.entries(aggregateByRole)) {
          if (count > 0) {
            ...
            console.log(
              `     ${role.padEnd(15)} ${String(count).padStart(4)} (${pct.padStart(5)}%)`,
            );
          }
        }
      }
    ...
    try {
      ...
      for (const session of sessions) {
        ...
        for (const msg of messages) {
          ...
          const currentRole =
            existingOverrides[msg.sourceKey]?.role ?? msg.messageRole;
          ...
          if (newRole !== currentRole) {
            ...
            if (options.dryRun) {
              console.log(
                `   [DRY RUN] ${msg.sourceKey.slice(-20)}: ${currentRole} → ${newRole}`,
              );
            } else {
              const override: RoleOverride = {
                role: newRole,
              ...
              console.log(
                `   ✓ ${msg.sourceKey.slice(-20)}: ${currentRole} → ${newRole}`,
              );
            }
          }
        }
