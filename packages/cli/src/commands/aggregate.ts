...
export const aggregateCommand = new Command("aggregate")
  ...
  .option(
    "--all",
    "Run all steps including linking (default: coverage + validation only)",
  )
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.warn(
        chalk.yellow(
          `Warning: Could not load profile pack, using minimal defaults`,
        ),
      );
    ...
    const { artifacts, lxArtifacts, entities, statements } = await loadRunData(
      iwDir,
      runId,
    );

    console.log(
      `Found ${artifacts.length} artifacts, ${entities.length} entities`,
    );
    console.log("");
    ...
    if (runLink) {
      ...
      try {
        ...
        console.log(
          chalk.dim(
            `Step 1: Using existing ${linkProposals.length} link proposals`,
          ),
        );
      } catch {
    ...
    if (!skipCoverage) {
      ...
      console.log(
        `  ✓ Traceability score: ${coverageResult.summary.traceabilityScore.toFixed(1)}%`,
      );

      if (verbose) {
        coverageResult.roleTransitions.forEach((rt) => {
          console.log(
            `    ${rt.sourceRole} → ${rt.targetRole}: ${rt.coveragePercent.toFixed(1)}%`,
          );
        });
      }
    } else {
    ...
    console.log("");
    console.log(
      chalk.green(`Output written to: ${path.relative(cwd, outputDir)}/`),
    );
    console.log("");
    console.log("Generated files:");
    if (runLink && !skipLink)
      console.log(`  ${chalk.blue("lx.proposals.json")} - Link proposals`);
    if (!skipCoverage)
      console.log(`  ${chalk.blue("coverage.json")} - Coverage report`);
    if (!skipValidate)
      console.log(`  ${chalk.blue("findings.json")} - Validation findings`);
  });
