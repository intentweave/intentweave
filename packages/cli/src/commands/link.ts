...
export const linkCommand = new Command("link")
  ...
  .option(
    "--matchers <list>",
    "Matchers to use: name,alias,structural,profile",
    "name,alias,structural,profile",
  )
  ...
  .action(async (options) => {
    try {
      ...
      if (artifacts.length === 0) {
        console.error(
          chalk.red("No artifacts with PX output found in this run."),
        );
        process.exit(1);
      }
      ...
      // Count entities
      const totalEntities = artifacts.reduce(
        (sum, a) => sum + a.entities.length,
        0,
      );
      ...
      // Write output
      const outputPath =
        output || path.join(runDir, "aggregate", "lx.proposals.json");
      ...
      console.log(
        `  ${chalk.blue(`${CLI_NAME} validate --run ${runId}`)} - Validate the run`,
      );
      console.log(
      ...
      console.error(
        chalk.red(
          `Link generation failed: ${err instanceof Error ? err.message : String(err)}`,
        ),
      );
