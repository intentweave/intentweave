...
export const docHealthCommand = new Command('doc-health')
  .description('Analyze documentation freshness — detect stale, drifted, and undocumented entities')
  .argument('[files...]', 'Document file(s) to analyze (omit to scan all session docs)')
  .option('-s, --session <id>', 'Session ID (required)', '')
  .option('--min-rel <n>', 'Min relationships for undocumented entity flagging', '2')
