...
describe("AstExtractor", () => {
  ...
  describe("extractFromString", () => {
    ...
    it("should extract interface declarations", () => {
      ...
      const idProp = result.symbols.find(
        (s) => s.name === "id" && s.parent === "User",
      );
      expect(idProp).toBeDefined();
      expect(idProp?.kind).toBe("property");

      const getDisplayName = result.symbols.find(
        (s) => s.name === "getDisplayName" && s.parent === "User",
      );
    ...
    it("should extract import statements", () => {
      ...
      const typesImport = result.imports.find(
        (i) => i.moduleSpecifier === "./types",
      );
  ...
  describe("utility functions", () => {
    ...
    it("getClasses should return all classes", () => {
      ...
      expect(
        classes.filter((c) => c.name === "A" || c.name === "B"),
      ).toHaveLength(2);
    });
  });
});
