...
describe('Starter Profile Pack', () => {
  ...
  it('has validation rules', async () => {
    ...
    const completenessRules = pack.rules.filter(r => r.id.startsWith('completeness-'));
    expect(completenessRules.length).toBeGreaterThan(0);
    
    // Check for consistency rules
    const consistencyRules = pack.rules.filter(r => r.id.startsWith('consistency-'));
  ...
  it('has proper transition shape with required predicates', async () => {
    ...
    const fromState = transitionShape?.predicates.find(p => p.name === 'FROM_STATE');
    const toState = transitionShape?.predicates.find(p => p.name === 'TO_STATE');
  ...
  it('has resource-has_state shape', async () => {
    ...
    const hasStatePredicate = resourceShape?.predicates.find(p => p.name === 'HAS_STATE');
